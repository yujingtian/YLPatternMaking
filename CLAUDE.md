...
#   sim 直渲 buildSimView；解算双停自然停
#   （2026-09-23 起 worker 帧驱动）；宿主不渲染，前片/袋贴/
...
#   （hold→lowering 前后裆独立探针缓释钉高俯仰涌现→settle→done；挂胯判据 P1 2026-09-23：
#   lowering 第三停径——候选档行周长 rowPerimeter ≥ 环长×(1+jamMargin 0.02) 即卡停不下放，
#   不可伸长闭环套不进更粗截面是精确几何事实，治掉裆拽环嵌体/深褶/波浪，口径 §10.11「挂胯判据」条；钉环随行重投影 P2 2026-09-23：下放中钉 XZ 沿 buildWaistRing 参考环随行（参考行下钳腰站行——裸逐钉行已两轮对照证伪）、弧位 s 直通防环向游移、环长恒 = 成衣腰长不动，正面腰头悬空 med +0.53→−0.07，口径 §10.11「钉环随行重投影」条；解算 Worker 化 2026-09-23：构建留主线程、
#   toSolveState 零拷贝进 worker 全速解算（solvestate/dressDriver/dressWorker，
#   双宿主锁步位级金标）+ 无损提速（sqrt/分箱/帧数压缩，单步 84.8→49.3ms）、
#   主线程交互零阻塞，口径 §10.11「穿台解算 Worker 化」条）、
#   DressReport 合身读数（掉裆/接触三态/
#   最差穿透 tooSmall/jamF·jamB 卡胯停——读数提示不改版型）、滑杆 ref 冻结+「重新试穿」手动闭环；脚碰撞修复
#   （场 yMin 下探罩脚底消除行表负 y 盲区 + 腿轴扫描止踝防脚环喇叭摆位）+ 长裤盖脚（2026-09-20：脚区幕帘摆位 placement.buildFootCurtain 踝下布沿脚面弧长走线〔脚口周长<脚底切片周长环抱不可能——脚口本就搭脚背，初值直落正确 drape 盆地〕+ collide 上表面竖直支撑 topSupportY〔阈 0.6 盖趾盒冠、含埋点救援，规格 §10.11「长裤盖脚」条〕）+ 热力图间隙通道
#   （heatmap.ts：间隙=布离体带符号距离，开关只重着色不重跑仿真；应变通道 2026-09-20 移除）+ 腰圈钉环
#   形随体长随衣+间隙均匀（2026-09-19；2026-09-20 等距外偏：腰站截面边界沿外法线等距偏移至
#   周长 = 成衣腰长，周身间隙均匀 |δ|〔旧绕原点缩放 gap∝|p| 前:后 5.5:1 环前骑已废〕，
#   偏小款 δ<0 均匀穿体、穿不进由带符号热力图 gap 红区读出；口径 §10.11）
