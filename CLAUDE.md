...
- [打版流程.md](打版流程.md) 是步骤的唯一权威来源；[.doc/](.doc/) 下每篇推导文档对应一类公式（臀围、裆、腰、腿、腰头、口袋、袋布、门襟、贴袋、毗围……），[.doc/python工程设计.md](.doc/python工程设计.md) 是工程设计文档，[.doc/工厂DXF逆向解析.md](.doc/工厂DXF逆向解析.md) 是工厂 DXF 反解析（`ylpattern reverse`）的口径权威。**文档先行**：部分特征先有推导文档、后程序化，属正常在建状态。
