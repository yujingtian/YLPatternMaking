...
build_fitting_payload(ctx, m) -> dict：前后片净样边链经局部 -> 整版全局
反变换（origin/frame 随 PatternPiece 携带，pieces.py）序列化到全局坐标
（cm、Y 向上），附人台放样站点（整版关键线高度 + 成衣围度）与腰头标量。
腰头为旋转局部系（origin=None、frame="local"），不逆推坐标、只随标量导出。
...
def _to_global(p: Point, origin: Point) -> list[float]:
    """裁片局部坐标 -> 整版全局坐标（frame="reflect_y" 的逆变换：
    local=(x−ox, oy−y) 故 global=(lx+ox, oy−ly)）。"""
    return [round(p.x + origin.x, 6), round(origin.y - p.y, 6)]


def _pts_out(pts: list[Point], origin: Point | None) -> list[list[float]]:
    """折线点列序列化：origin=None（局部系裁片）原样输出。"""
    if origin is None:
        return [[round(p.x, 6), round(p.y, 6)] for p in pts]
    return [_to_global(p, origin) for p in pts]


def _piece_entry(piece) -> dict:
    """单个裁片序列化。frame="reflect_y"（前后片）反变换到全局坐标；
    frame="local"（腰头）保持原样。"""
    roles = _EDGE_ROLES.get(piece.name, {})
    origin = piece.origin
    ...
    if piece.grain is not None:
        grain = _pts_out([piece.grain.a, piece.grain.b], origin)
...
def build_fitting_payload(ctx: DraftContext, m: Measurements) -> dict:
    ...
    return {
        ...
        "pieces": [_piece_entry(front), _piece_entry(back), wb_entry],
    }
