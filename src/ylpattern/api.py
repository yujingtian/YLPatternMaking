...
def _coerce_front_piece_sa(v) -> FrontSeamAllowances:
    """dict -> FrontSeamAllowances（已是其类型则原样返回）。"""
    if isinstance(v, FrontSeamAllowances):
        return v
    if isinstance(v, dict):
...
def run(*, waist: float, hip: float, knee: float, hem: float,
        ...
        front_piece_seam_allowances: dict | object | None = None,
        front_piece_crotch_corner: bool = True,
    ...
    参数：
        ...
        front_waist_dart 前片腰长调节量 V前省（纯调节，不对应绘制的省；标准牛仔裤 0；
                        西裤褶裥款 1.5~3.0；袋口吃省 ΔW 另自动计入腰长）
        back_waist_dart 后片腰长调节量 V后省（约克转移量，约克步骤前 0；Yoke 2.5~4.0；
                        纯调节不含绘制腰省宽，Σ腰省宽自动计入后腰长）
        ...
        front_pocket_dart_width  腰头吃省总宽 ΔW（cm，前口袋绘制.md §三.1，默认 0 不吃省，
                           需要时显式录入 1.5~2.5；ΔW 按腰长不变量自动加进前腰长（缝后腰围
                           恒等规格，不被吃省吃掉）；
        ...
        front_piece_seam_allowances
                          前片裁片九边独立缝份 dict {waist,rise,inseam,side,hem,
                          mouth,fly_top,fly_outer,fly_bottom}（cm，前片裁片.md
                          §2.1；waist 装腰 1.0、rise 前浪 1.0、inseam 下裆 1.0、
                          side 侧缝 1.5、hem 裤口卷边 2.5、mouth 袋口挖削 1.0、
                          fly_* 连裁门襟三边 1.0）
        front_piece_crotch_corner
                          前浪浪尖（裆尖）缝份角形态开关（§2.2 两态；默认
                          True 外凸镜像折角补偿缝份翻折长度差防"缺肉"；
                          False = 前浪缝边与下裆缝缝边各沿端切线直线延长
                          相交于单一顶点（切线 miter，用户口径 2026-08-24））
    ...
    o = PatternOptions(delta=delta,
                       ...
                       **({"front_piece_seam_allowances":
                           _coerce_front_piece_sa(front_piece_seam_allowances)}
                          if front_piece_seam_allowances is not None else {}),
                       front_piece_crotch_corner=front_piece_crotch_corner,
    ...
    if not until:
        ...
        # 传 pieces_dxf 或任一裁片 SVG 时经 collect_pieces 固定顺序全收集
        # （推码方案步 2：收敛 api/cli 平行分支），按片名映射写 SVG、末尾一并出 DXF
        svg_map = {
            "waistband": waistband_svg, "back_yoke": yoke_svg,
            "front_facing": front_pocket_svg, "front_patch": front_pocket_svg,
            "front_pouch": front_pouch_svg,
            "front_fly_single": front_fly_single_svg,
            "front_fly_double": front_fly_double_svg,
            "watch_pocket": watch_pocket_svg, "belt_loop": belt_loop_svg,
            "back_patch": back_patch_svg, "front_piece": front_piece_svg,
            "back_piece": back_piece_svg,
        }
        if any(v for v in svg_map.values()) or pieces_dxf:
...
def _last_residual(trace_text: str) -> float | None:
    """从 closure 返回的追踪文本解析末轮毗围残余 ΔW（无闭环记录返回 None）。"""
    import re
    matches = re.findall(r"ΔW = ([+-]?\d+(?:\.\d+)?)", trace_text)
...
def run_size_run(size_file: str, *, pieces_dxf: str,
    ...
    if residuals:
        print("毗围闭环逐码汇总：")
        for label, dw, n in residuals:
            state = ("已收敛" if abs(dw) <= o.thigh_tol
    ...
    if trace:
        with open(trace, "w", encoding="utf-8") as fp:
            fp.write(base_trace)
        print(f"追踪记录已输出:{trace}（基码 {run.base}）")
    if report:
        from .exporters import report as report_exp
        with open(report, "w", encoding="utf-8") as fp:
            fp.write(report_exp.render_report(
                contexts[run.base].sheet, run.base_measurements(),
                contexts[run.base].options, base_trace))
        print(f"报表已输出:{report}（基码 {run.base}）")
    return contexts
