...
阶段 8：前口袋 + 袋贴（可选步骤）-- 打版流程.md「前口袋打版过程」（前口袋绘制.md §二、§三、§三.3 袋贴）。
阶段 9：袋布（可选步骤，依赖前口袋）—— 打版流程.md「袋布打版过程」（袋布绘制.md §二、§三、§五）。
阶段 10：门襟（可选步骤；连裁门襟上版于前片，独立门襟叠画于前片后另走
裁切链 front_fly_flow 出独立裁片）—— 门襟绘制.md §2.1~§5。
"""
...
FRONT_FLOW = [
    ...
    # -- 阶段 8：前口袋 + 袋贴（可选步骤，开关开启才绘制；先画后裁，不动其他元素） --
    fps.draw_front_pocket,
    fps.draw_front_pocket_facing,   # 袋贴（依赖前口袋主切口，§三.3；先画后裁）
    fps.draw_front_watch_pocket,   # 小表袋（依赖前口袋挖削嵌入式）
    fps.draw_front_patch_pocket,
    # —— 阶段 9：袋布（可选步骤，依赖前口袋主切口） ——
    fph.draw_front_pouch,
    # —— 阶段 10：门襟（可选步骤；连裁门襟上版于前片，独立门襟叠画后另走
    #     裁切链 front_fly_flow 出独立裁片） ——
    ff.draw_front_fly,
]
