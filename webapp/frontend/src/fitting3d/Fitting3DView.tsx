...
export default function Fitting3DView({
  ...
  // rise 缝合对 + 腰口整圈全向钉直挂 + 地面碰撞，无芯碰撞——真实提
  // 着整个腰口的形态：顶缘按摆位弧全撑、中缝竖直在前中/后中不塌轴、
  // 布自顶缘垂落）；前片/袋贴
  ...
  useEffect(() => {
    ...
    try {
      ...
      gv = view
      // 悬挂链（同步构建，无 async 竞态窗口）：芯/场仅用于初摆位半径与
      // 取景包络；sim 自由垂（无芯碰撞——芯撑出的前凸筒不是真实提着
      ...
      // 场景中心 = 人台左沿 ~ 平铺组右沿的中点（双主体：人台+悬挂筒 / 平铺）
      let flatW = 0
