// 水平切片：三角形与 y=const 平面求交 -> 交线段 -> 闭环链（围度测量与
// 高度场 splat 共用）。Python 侧同构实现 scripts/vendor_makehuman.py::girth_at
// （数值口径一致：交点按无序边缓存共享——相邻三角形切缝顶点同对象，
// 链环按点身份闭合，无浮点端点匹配）。
// 约定：d = y_vertex − y_slice，d ≤ 0 记「下」；跨面三角形恰有 2 条
// 符号相异边（顶点恰在平面上计入「下」，与 Python 一致）。
...
// 站点围度：per='body' 全环之和（腰/臀单环）；per='leg' 取质心 x>0 的右腿环。
// 只信闭环（开链是撕裂态，围度无意义）
export function stationGirth(
  pos: Float32Array, idx: Uint32Array, y: number, per: 'body' | 'leg',
): number | null {
  const loops = sliceLoops(pos, idx, y).filter((l) => l.closed)
  if (!loops.length) return null
  const sel = per === 'leg' ? loops.filter((l) => l.cx > 0) : loops
  if (!sel.length) return null
  return sel.reduce((s, l) => s + l.girth, 0)
}
