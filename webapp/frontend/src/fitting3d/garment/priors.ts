...
export const HANG_PRIOR = {
  ...
  hangLift: 8,           // cm：悬挂解算整体抬升（八期整裤开芯碰撞后重定标）：
                         // 整裤自腰口钉挂全长 ~99cm，PBD 约束在持续重力下
                         // 拉伸 ~4cm（lift 4 实测 hem 落地）——抬 8 后下摆
                         // 终态离地 ~3~4cm 不拖地（三夹具实测）。原 12 是
                         // 自由垂单筒标定。作用于摆位 y（含钉目标），drape
                         // collide 截面查询按 yLift 回减，地面碰撞安全网
