...
export const HANG_PRIOR = {
  ...
  legReparam: true,      // 八期整裤：腿区（fork = 各宿主裆尖 y 以下）逐高度
                         // 局部归一摆位开关——内边界（inseam 链）精确落
                         // 前/后中 pinch 线（θ=0/−180°，芯花生环 ±z 腰谷）、
                         // 外边界（side 合链）落 ±90°；false 回退全局 x→θ
  forkBlend: 0,          // cm：fork 上下躯干/腿两套摆位映射的线性混合带
                         // 半宽（0 = 硬切换；fork 高度出现折痕时 ±3 备用
                         // 旋钮，单 knob 单变量）
} as const
