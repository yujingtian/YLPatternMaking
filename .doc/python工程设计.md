...
**前端（webapp/frontend/src/fitting3d/，Fitting3DView + bodymesh/{bin,morph,types,slice,height} + garment/（试穿），依赖 three；garment 另加 delaunator + three-mesh-bvh（worker））**：
...
- **悬挂链（2026-09-14 定型，试穿显示口径退役为悬挂；演进史归决策日志 §十一 当日条）**：useDraft.generateFitting（protocol `'fitting'` cmd 引擎优先回落 HTTP）→ `garment/` 纯函数层 → `garment/worker/`（sim worker 自建 mesh/warp/场/BVH/整裤 + band snap 实际曲面 + 自驱 60Hz stepSim——**解算在纸样系进行（y=纸样高、hem≈0 落地、warp=identity），碰撞体 = 纸样围度撑型芯**（core.ts，2026-09-15 解耦定型：girth_finished 逐站取值、芯半径 = g/2π − skin → 布面恰落纸样围度，圆筒按构造成立、廓形=版型；单一水密花生网格——腰臀圆环 + 裆下双瓣花生整环〔瓣周长锁定 2×腿站 girth、腰谷 ε 平滑、裆带 ±3cm 圆↔花生混合；**裆站躯干圆半径 = coreRadius(max(臀, 2×腿))**（毗围代理——布在叉口连续；直取单腿半径会把臀→裆掐成蜂腰再弹回双瓣，「大腿比臀大」的结构放大器）〕，v1 圆筒+双管+龙骨三件套的四条死路见 core.ts 头注：留槽内缝捷径/相切刀口泵/交叠符号陷阱/细龙骨穿膛）；协议 `hang` 标志 = **帧数封顶兜底** `HANG_PRIOR.maxFrames`=600，超限强制取当前帧出 settled〔形状 f150 已达标：缝 0.03/穿透 0/包腿 wrapKnee≥0.7〕防 basin 彩票永不收敛卡死显示。**两个已证伪方案勿回头**：①无碰撞单帧——腿间空隙无碰撞体，内缝收缩把裤腿对折塌陷〔包腿圆度 wrapHem 0.50，用户截图实证〕；②全钉腰头 pinYOnly=false——腰环钉死反而阻止滑移穿衣，穿透 18cm 卡在腿间空隙〔穿衣时代"锚定冻旋转=假收敛"教训在悬挂同样成立〕）。**显示层偏移**：worker 几何空间 = 纸样系（与 payload 同源）〔pins/crotch 目标自洽〕，视图把裤子 Group `position.x = 2·BodyField.maxRadius + garmentGap + HANG_PRIOR.clearance(6)`、`position.y = HANG_PRIOR.hemLift(2)`（纸样 hem 站 y=0 恰落地，裤口边与地面格网共面打架 = 用户截图裤脚「白色交叉线」观感；抬离地面亦贴近真实悬挂陈列）平移到人台 +X 侧撑开悬挂；**初始静态裙形（缝未闭）visible=false**，首帧起播放成形动画〔终态圆筒，约 4~5s〕；弯腰头回退环同偏移；相机 target/机位按 offset/2 差量平移居中双主体〔幂等〕，flyTo 四预设同偏移。**独立原则（用户拍板）不变**：人台滑杆不改衣服、打版参数不改人台；合体度只经松量读数表达（**应变热力图随悬挂退役**——UI 开关与接线删除，heatmap.ts/render.setStrain 留档不触发）；交互 = 重新悬挂（重发 payload+热启动）/重置解算双按钮 + 自动重挂开关（默认关，600ms debounce；关=bodyStale 提示+手动重挂 link）；**绝无自动调版/自动改体型闭环**。模块口径：core（撑型芯：纸样围度逐站成芯、单一水密花生网格、布面恰落 girth_finished——见 core.ts 头注）；mesh（边界弧长重采样+错排栅格+delaunator 质心过滤；内部三角形最小角 >15°、面积比 ≥99%，边界细长三角形旧链原样）；band（直腰头且**无挖削** = scalars bottom_length/width 前端重建真实布片与前后片物理缝合——用户口径「腰头一定要和前后片连在一起」；对账偏差 >0.5% 炸；弯腰头或口袋挖削款回退**视觉环带**——弯腰头净样非矩形三标量重建不出、挖削款腰口弧缺段+袋布桥无片可缝对不上账；环带沿**腰口 pin 弧逐点成环**（buildWaistRing：各片 top_chain 摆位点 θ 排序成底圈 + 腰头宽顶圈，真实腰口弧高低/半径逐点跟随——平环挂 field+1.5 会悬空出「腰头没连一起」的缝）+ 界面提示）；align（y-warp 五节点分段线性，站名对齐表**显式写死**勿按名匹配——引擎 hip/hem vs 人台 hips/ankle）；placement（圆柱包裹 θ 映射+支撑半径场 R(y,θ) 凸上界 + garmentGap 1.2 松量悬垂起步；band 行高 = 腰站锚+真布高 bandWarp；右半镜像 θ→−θ；**back/yoke 用 1−t 反向走扇区**——后片 CB 在高 x，正向映射会把侧缝/后中放反 = 扭转错位根因，已修）；seams（**物理缝合配对按弧长参数**——无 yoke 11 条〔side×2/inseam×2/rise/cb 镜像/腰缝×4/腰头自身后中〕；有 yoke 16 条〔侧缝链式拆分×4：前片侧缝全长按弧长对开，下段 [1,1−fb] 配后片（脚口端对齐）、上段 [1−fb,0] 配育克——**上段起点 1−fb 勿按 fy（yoke 边长/前片长）起配**：fy+fb≈1.006>1 把交界 ~0.6cm 条带双重预订到交汇点上下两侧（目标相距 ~1.2cm），四交汇点永久拔河=环向蠕动泵永不收敛；0.61% 侧缝净长和差按 M1 同口径全段对全段分数配对均匀吸收；+ 育克 cb 镜像×2 + 育克下口↔后片上口×2 + 腰缝×4（后侧改缝育克上口，环序 [yoke_L, front_L, front_R, yoke_R]）〕；**口袋挖削款（前片 mouth 边存在）侧缝改道**：前片侧缝全长只配后片侧缝**下段**（1:1 自脚口端），后侧链余量（育克侧边+后片上端）配**袋贴侧边**（腰口端锚、沿链推进——前片全长对后片全长会把 P2 硬拉到后侧链顶 = 口袋区 ~4cm 剪切错位）；袋贴 = 第 5 布片（front_facing mesh L/R 双侧，走 front 扇区摆位但 t 用**前片 x 全程**归一——按自身 bbox 会把窄条拉伸铺满 90° 扇区）、**整片钉摆位**（衬布式贴合：芯体填满筒内，垂坠动力学只会与筒面抢壳 z-fight，「缝在前片正确位置」展示语义优先）、袋口 mouth 边保持 free（口袋开口），缝合对是约束不是共享顶点；pin = 腰头上沿 belt line **只钉 Y**（回退模式 = 各片腰口全向 pin + 袋贴整片钉）；裆尖 4 粒 crotchHold 60 帧硬释放；markDenimBend 裆/膝 ±6cm 窗增硬 0.5；fly 连裁显式拦截报错）；solver（旧 PBD verlet+Gauss-Seidel 3×6 原样复活 + preRelax 300 拉链渐进 + 发散兜底恢复好帧/冻结）；collide（three-mesh-bvh；**判内外后才决定 skin 跳过**——体内深陷粒子必须穿面推出且同步 prev 吃 teleport 速度踢〔旧 `distance≥skin→continue` 体内失明已修〕；penetrationStats/meshDepth = 网格口径穿透验收）；ease（松量切片：整裤围度容差 greedy 链 tol 1.5 跨缝闭环、挑环同 readGirth 口径 body=最大环/leg=最右大环、crotch 分叉站跳过；解耦后整裤侧在纸样系 st.y 切、人台侧 warp 后切——两侧各读各的坐标系）；heatmap+render.setStrain（随悬挂退役留档不触发）。**调参红线（全档 priors.ts 注释 + 决策日志当日条）**：启动序列对收敛 basin 敏感勿盲调——band rest 全保纸样矩形（重定=预应力喇叭）、preRelax 300（450 缠结吸引域）、crotchHold 60 硬释放（40/180/240-fade 全差）、启动强阻尼适得其反、丹宁 0.45~0.5 **助收敛**（0.3 全局=蠕动地板 ~0.8 擦阈值；0.6=磨转极限环 3.4 不收敛；span=4 窄硬环=铰链蠕动，6=稳定窗）、collisionSkin **0.98 持续接触锚**（0.3=M1 单裤时代旧值；yoke 整裤 16 缝在 0.3=整裤摆振极限环地板 10.6 不收敛——整裤 XZ 刚体模态〔pinYOnly 只锚 Y〕零刚度+GS 走带注入，skin 抬厚让贴体静止点全落接触壳、体表正压/摩擦锚住刚体模态真衰减；标定窗 [0.98,1.02] 双 fixture 全过〔0.95/1.05 双破，basin 非单调勿线性外推〕；damping/iterations/friction 单 knob 与 iters8 组合全反证，死路表录决策日志 2026-09-14 当日条；**窗口只对本 fixture 对成立，换体型/裁片需重验**）。M1 验收金标：11 缝 avg<0.3/P95<0.8、穿透 ≤0.15、settle ≤360 帧（实测 273 @ skin 0.98；BVH 接触噪声地板高于旧解析三管的系统性偏差已留档）。yoke 整裤 16 缝用例正常保留（2026-09-14 短暂误 skip 后当日解封：交接文档「不收敛」系攻坚中途所写、未回头更新——工作树〔侧缝链式拆分 + collisionSkin 0.98〕实测 M1 274/yoke 252 帧收敛，解封后 3/3 绿；教训：交接文档与决策日志冲突时以决策日志终态为准）。**悬挂金标 worker/hang.test.ts**（解算同穿台口径跑满：**包腿圆度守卫** wrapKnee≥0.65/wrapHem≥0.55〔实测 0.71~0.88/0.63~0.79；塌片=0.50 即无碰撞单帧翻车值——用户截图实证后新增的守卫〕+ 缝/穿透/600 帧内收敛）。**残留**：悬挂观感人工视检/VLM 复调（穿台时代摩擦 0.5 与丹宁数值 VLM 复调随退役挂起）。**2026-09-14~15 解耦攻坚修复记录（headless 复刻钉死后逐个击破，演进全录决策日志；下为现行口径）**：①**后片/育克摆位反装已修**（placement.placePoint back/yoke 用 1−t 反向走扇区）——旧 t 正向把侧缝放到后中、CB 放到左侧，cb 镜像对/侧缝对初始差 30~35cm，闭合靠绕体/穿体拧转 = 用户截图扭转错位根因（前片映射本来就对；金标 wrap/seam 指标测不出扭转）。②**collide 体内失明已修**：旧 `distance≥skin→continue` 把深陷体内粒子静默跳过（无界查询只修了命中没修过滤，inside 分支不可达），缝合把布拖进瓣心 8cm 也无人推出；现判内外后才决定跳过，且**体内穿面推出时同步 prev**（吃掉厘米级 teleport 的速度踢，防全场 churn）。③**碰撞体换撑型芯**（core.ts），人台完全出圈：init 不带人台几何（protocol/entry/useGarment 同步瘦身），Fitting3DView 撑型芯场摆位、autoDress/bodyStale 重挂 UI 退役（独立原则贯彻到解算）；ease 读数整裤侧在纸样系 st.y 切、人台侧 warp 后切（各自坐标系）。④**标定迁移**（priors）：friction 0.5→0.8（撑型芯上裤筒绕瓣无重力回复、0.5 周向蠕动地板 ~9 永不收敛；0.8+SEG64+ε0.3 组合收敛）、preRelaxCollideEvery 10→2（芯体细部拉链穿膛）、settleSpeed 0.8 保持（地板 M1 0.72 结算/直筒单 0.83 波动贴线由封顶兜底）。⑤**封顶诚实化**：frame 消息带 capped 标志，UI「已悬挂（600 帧未静止，已取终态）」不再伪报。⑥**穿透验收换网格口径**（collider.penetrationStats/meshDepth，面法线判向）：hull 场口径对花生腰谷误报（谷内布在 hull 内不在网格内）；现行金标断言网格穿透 0（实测双 fixture + 直筒单全 0）。**残留**：直筒单（curved 回退链）地板 0.83 波动贴线靠封顶出帧（视觉静止）；悬挂观感人工视检/VLM 复调挂起。金标 garment.test.ts / worker/solver.test.ts / worker/hang.test.ts / ease.test.ts（vitest node 直跑）。
