...
### 10.11 3D 人台显示（MakeHuman 全身原生 target 滑杆试验场；试穿链 + 围度闭环 2026-09-12 退役）

**定位**：右栏 3D 主视图常驻人台（进系统即有）= **MakeHuman 全身原生 target 滑杆试验场**（2026-09-12 用户口径：**全身**模型 + 用它自己的 targets 调节）——raw.obj 全身 A-pose 基网格（含头/臂；= vendor base.obj 的 git 内拷贝）+ 四部位双极滑杆（腰/臀/大腿/膝，−1..+1）直接驱动 **8 个官方 measure target** 权重，拖动实时 morph + 站点围度读数。与裁切链（base.bin/派生场）的分野：此处是**裸原生体验**——.target 索引即 base.obj 原始顶点号，与全身网格天然对齐、零重映射；原生场固有缺陷（thigh 场膝上死区等）属作者化行为，原样呈现。**无自动化闭环**（围度闭环/纵向对齐/体型库已退役）：滑杆值就是场权重 w（作者化域 [0,1]，试验场放开到 ±1 供试探）。演进史（试穿布料/PBD/碰撞 → 体型调节闭环 → 静态 raw.obj 直显 → base.bin 滑杆 → 全身原生滑杆五次转向；七轮形体打磨、六轮根治、VLM 验收全记录）见决策日志 §十一；恢复闭环/试穿 = 前端 revert、引擎零改动。
...
**前端（webapp/frontend/src/fitting3d/，Fitting3DView + bodymesh/{native,morph,types,slice}，依赖仅 three）**：
- **现行链路**：`Fitting3DView`（three 惰性分包；场景/格网/OrbitControls/视角预设/截图/显示开关；无 props、零 payload）→ `loadNativeBody()`（native.ts，模块级 promise 缓存）：fetch `public/bodymesh/raw.obj` 文本解析——**全部 v 行平铺保留原始顶点号**（target 直接按 base.obj 顶点号写，不紧缩；helper-*/joint-* 辅助顶点保留在数组里但不渲染），面只取 `g body` 扇形三角化（与 vendor parse_obj 同口径），dm→cm ×10，body 面引用顶点最小 y 落地脚底 y=0；并行 fetch 8 个官方 measure target（`public/bodymesh/targets/measure/*.target`，文本 `idx dx dy dz` dm 单位，`parseTargetText` 同 ×10）→ BufferGeometry 直显 → 滑杆 onChange：双极值映射场权重（正推 `site+`、负推 `site−`，两场独立作者化非反对称）→ `morphPositions`（稀疏叠加 pos = base + Σwᵢ·Δᵢ，签名只吃 `{positions, targets}` 结构）in-place 写 position attr + computeVertexNormals → `readGirth` 四站围度读数。初始机位与视角预设按 mesh 身高定标；加载失败 Empty 提示。**对齐/围度闭环/钳位不复位**：滑杆是裸权重，不经过 retired 链任何守卫。
- **站高探测与读数口径（近似，读数展示用非解剖地标）**：`detectStations` 自下而上逐 cm 切片扫 2→1 大环跳变定裆（BIG=25cm 环过滤剔除 A-pose 指段/小臂环），膝=0.57×裆、大腿=裆−6、臀=裆+10、腰=裆+23 比例派生；探测失败静默置空（读数显示空白不炸）。`readGirth`：body 站取**最大环**（躯干恒大于挂臂环）、leg 站取**最右大环**（右腿——躯干质心可能微偏正，按 cx>0 过滤会被大围度躯干误选）。站高取 w=0 值，native 场微移站高，极端权重下读数有轻微口径偏差，试验场可接受。
- **资产与许可**：数据源 `vendor/makehuman/`（base.obj + targets + CC0 全文 + PROVENANCE.md 记 repo/tag/commit/sha256；`.gitattributes -text` 禁 EOL 转换保哈希可校验）；运行时消费 `public/bodymesh/raw.obj`（= base.obj 的 git 内拷贝，sha256 一致）+ `public/bodymesh/targets/measure/` 8 个官方场原样拷贝（LICENSE.ASSETS.md 同目录）。许可红线（用户拍板）：仅商用安全许可（CC0/MIT/Apache）、MakeHuman 只借 CC0 数据绝不移植 AGPL 代码、SMPL 出局。`scripts/vendor_makehuman.py` 派生的 `base.bin/targets.json` 保留（Python 金标 `tests/test_vendor_bodymesh.py` 把守，前端路径二度退役闲置）。
- **测试**：`bodymesh.test.ts`（parseTargetText 文本契约 dm→cm + morph w=0 逐位恒等/稀疏叠加/源数组不被改写）；`native.integration.test.ts` **真数据冒烟**（盘上读 raw.obj + 官方 target，把守 g body 面可解析/身高 150~200cm/站高四站有序/基线围度解剖区间/target 索引域不越顶点数——heuristic 漂移即炸；为此 devDep 加 @types/node，`import.meta.dirname` 定位）。引擎金标 `python -m pytest tests/test_fitting_payload.py tests/test_vendor_bodymesh.py -q` 照旧。

**（史实存档）退役清单（2026-09-12 同日多批；机制细节、金标口径、验收复验全录决策日志 §十一）**：①试穿链——布料四半片/腰头环带/结构线/应变热力图/PBD 解算/三管高度场碰撞（`pbd/`、`seams.ts`/`mesh.ts`/`structureLines.ts`/`heatmap.ts`/`useFittingSolver.ts`、`bodymesh/heightfield.ts`）与 mannequin 适配层（`mannequin.ts`）；②体型围度调节链——`BodyProfile`/体型库 UI/localStorage `ylpattern.body.v1`、`bodymesh/` 运行时（load/morph/align/calibrate/build）、`useMannequin.ts`、`priors.ts`、fitting payload 前端消费（`postFitting`、protocol `'fitting'` cmd、`types.ts FittingResult` 契约、useDraft `generateFitting`）与「生成」按钮/StaleFlag。③静态 raw.obj 直显（rawMesh.ts）同日让位滑杆试验场。④base.bin 前端滑杆路径（bodymesh/load.ts + parseBin）半日后让位全身原生链（用户口径：半身裁切网格非所愿，全身 + 官方 targets 裸体验）。更早史实：旧环模型（SDF smin 蒙皮 + 环放样七轮打磨）2026-09-11 随真人网格换轨整体退役。
