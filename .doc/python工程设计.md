...
一期范围：参数录入（分组面板）-> 整版/裁片 SVG 预览 -> 裁片合集/整版 DXF 下载 + 尺寸单 toml 导出 + 模板载入；无账号/无拖拽（可调点配置表已定格式占位）。启动：`pip install -e ".[web]"` 后 `uvicorn webapp.backend.app:app`（前端 dist 已构建并由 FastAPI 托管；改前端走 `cd webapp/frontend && npm run dev`，Vite 代理 /api）。
...
- **折叠默认态（2026-08 用户口径）**：参数面板默认仅展开「基础测量」组，其余全收起（antd Collapse defaultActiveKey=['measurements']，schema 组的 collapsed 字段已不参与默认态）。含错误的组靠标题 Badge 计数提示，用户自行点开。
- **参数显示名口径（2026-08）**：前端参数名一律显示英文 key（用户口径），schema.py 顶部 `USE_CN_LABELS = False` 总开关，置 True 恢复源码注释解析的中文标签；分组标题/界面文案不受影响。
- **前端组件库（2026-08）**：Ant Design 5（zh_CN locale、主题色 #2c6e49），组件映射--分组面板=Collapse（默认全展开、misc 收起、错误组挂 Badge 计数）、参数行=Switch/InputNumber/Select（缝份=小网格、tuple=JSON 文本 Input）、裁片预览=Tabs card 式（原 tab 裁片被关闭时回退整版）、校验=Alert 横幅、下载失败=message.error、模板=Select；AntApp 包裹供 useApp 取 message 实例。antd 全量引入 bundle ~694KB（gzip 228KB），内部工具可接受，未做按需分割。
- **启动坑**：webapp/backend/app.py 含包内相对导入，cd 进 backend 目录用 `python app.py` 直跑必报 "attempted relative import with no known parent package"；唯一正确姿势是项目根目录 `python -m uvicorn webapp.backend.app:app`（app.py 末尾 __main__ 守卫会拦截误用并给出指引）。
