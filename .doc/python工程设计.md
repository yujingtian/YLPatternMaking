...
一期范围：参数录入（两段式分组面板）-> 两步生成（整版生成/裁片生成，2026-08-25）-> 整版/裁片 SVG 预览 -> 门控 DXF 下载（裁片合集/整版，对应步骤已生成且未过期才可下）+ 尺寸单 toml 导出 + 模板载入；无账号/无拖拽（可调点配置表已定格式占位）。启动：`pip install -e ".[web]"` 后 `uvicorn webapp.backend.app:app`（前端 dist 已构建并由 FastAPI 托管；改前端走 `cd webapp/frontend && npm run dev`，Vite 代理 /api）。
