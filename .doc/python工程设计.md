...
- **测试**：`fitting3d.test.ts`（mannequin 适配层 radiusAt/sectionAt/estimateBody 金标/体型库/欧拉公式 V−E+F=1/摆位约定/碰撞投影/全 pin settled/应变单调）+ `bodymesh.test.ts`（合成网格金标：slice 切片/morph 稀疏叠加/align 分段 y-warp（含踝节点路径）/calibrate 差分雅可比反演+耦合联立（waist+ 溢出 hips 站一并满足、hips 权重保持 0——旧对角反演在 hips+ 行 null 时永不收敛）+分档钳界（合成资产含 waist+/thigh+ 双 target）/heightfield 三管一致/buildMeshMannequin 出口契约——node 无 fetch/public，**真数据金标在 Python 侧** test_vendor_bodymesh.py）+ `fitting3d.integration.test.ts`（真资产端到端：node:fs 读 public/bodymesh 走与浏览器同一 parseBin→buildMeshMannequin 链 → 网格拓扑→缝合配对→150 帧 PBD 有限/缝合闭合<1cm/不穿透（+skin−0.15 强断言））。fixture 重生成命令见 integration 文件头；fixture 用静态 `import fx from './_fixture_fitting.json'`（tsconfig 开 `resolveJsonModule`）——**产品代码勿用 `node:fs`**：tsc -b 浏览器 lib 无其类型，`npm run build` 的 tsc 阶段报 TS2307（2026-09-09 实测；测试文件内可以 `// @ts-expect-error` 紧贴 import 行压制）。
