...
### 10.11 3D 人台显示（MakeHuman 下半身切割+站直 target 滑杆试验场；试穿链 + 围度闭环 2026-09-12 退役）

**定位**：右栏 3D 主视图常驻人台（进系统即有）= **MakeHuman 下半身站直人台 + 官方 target 滑杆试验场**（2026-09-13 用户口径：**只显示下半身**、模特站直姿势、形态调节用它自己的 targets）——base.bin = 切割+站直姿势链产物（站直姿势：官方 rigs 骨架+蒙皮权重 clean-room LBS、大腿/小腿/脚逐关节角度、髋-膝-踝三点铅垂、脚底回水平，粗裁去臂、精裁腰+15、封盖水密；官方 14 场 = 12 measure〔腰/臀/大腿/膝/小腿/踝〕+ 身高 macro ±〔`macrodetails/height/` max/minheight〕已按切割后索引重映射 + 增量随帧旋转，身高场 ΔH 切割前实测写 meta.height），六部位双极滑杆（腰/臀/大腿/膝/小腿/踝，−1..+1）+ **预设体型芯片（155/160/165/170/默认）+ 身高滑杆（cm 连续、钳常人窗 150~185、可停任意身高如 162.3）**直接驱动场权重，拖动实时 morph + 站点围度读数（站高 = vendor 地标检测值 × 身高因子）。原生场固有缺陷（thigh 场膝上死区等）属作者化行为，原样呈现。**无自动化闭环**（围度闭环/纵向对齐/体型库已退役）：滑杆值就是场权重 w（作者化域 [0,1]，试验场放开到 ±1 供试探）。演进史（试穿布料/PBD/碰撞 → 体型调节闭环 → 静态 raw.obj 直显 → base.bin 姿势链滑杆 → 全身原生滑杆 → 纯切割 base.bin 滑杆 → 小腿/踝引入 → 站直姿势 LBS）见决策日志 §十一；恢复闭环/试穿 = 前端 revert、引擎零改动。

**引擎侧（exporters/fitting.py，纯 stdlib——前端 2026-09-12 起不再调用，出口/端点/金标照旧闲置）**：`build_fitting_payload(ctx, m)` 自调 `build_front_piece`/`build_back_piece`/`build_waistband`，输出 schema v1：`body{stations, points, crotch_drop, waistband_width, waistband_type, outseam}` + `pieces[front, back, waistband]`。`PatternPiece` 尾部两个 `origin/frame` 默认字段（裁片携带）与三个重建点透传（`pieces.py` + `cutter.py`）一并保留（消费方在前端已灭，引擎契约不动）。
...
- **边角色表**（引擎现行输出；消费方已退役，仅供恢复参考）：front `waist:top_chain, hem:hem, mouth/fly_*:free, rise/inseam/side:seam`；back `waist:top_chain, hem:hem, cb/inseam/side:seam`。`girth_finished` 均为成衣量（waist/hip 整圈、thigh/knee/hem 单腿）。
- **双通道**：`POST /api/draft/fitting`（app.py）与 `engine_glue._fitting`/`_COMMANDS["fitting"]` 逐字段复刻，`test_engine_glue.py` 钉全等（两通道均无前端调用方）。改引擎后照常 `npm run build:engine`。

**前端（webapp/frontend/src/fitting3d/，Fitting3DView + bodymesh/{bin,morph,types,slice,height}，依赖仅 three）**：
- **现行链路**：`Fitting3DView`（three 惰性分包；场景/格网/OrbitControls/视角预设/截图/显示开关；无 props、零 payload）→ `loadBodyMesh()`（bin.ts，模块级 promise 缓存）：fetch `targets.json`（**`cache:'no-cache'` 带 ETag 再验证——它是击穿链的链头，裸 fetch 会被浏览器启发式缓存钉死旧指纹、指向旧 base.bin，vendor 重跑后整链失效**）→ 以其 `baseSha256` 指纹拉 `base.bin?v=<sha8>`（vendor 重跑后指纹必变即击穿浏览器缓存）→ `parseBaseBin` 二进制解析（布局头注 + 尾部残留/索引越界防爆）+ 与 targets.json 计数对账 → BufferGeometry 直显 → 滑杆 onChange：双极值映射场权重（正推 `site+`、负推 `site−`，两场独立作者化非反对称；身高同构——heightW 正推 `height+`/负推 `height-`，**预设芯片 = `weightFor` 按 meta.height 实测 ΔH 把目标 cm 换算成权重一键设值（155~170 落 w=−0.34~+0.04），身高滑杆 cm 值同一换算反向驱动**；换算纯函数 height.ts：heightCm/weightFor 互逆 + ±1 钳制 + stationFactor）→ `morphPositions`（稀疏叠加 pos = base + Σwᵢ·Δᵢ，签名只吃 `{positions, targets}` 结构）in-place 写 position attr + computeVertexNormals → `readGirth` 六站围度读数（**站 y × stationFactor = heightCm/baseCm 身高因子——身高场近似等比升降全身，围度站随身体移动；比例近似口径，与「场会微移站高」caveat 同级**）。初始机位按裁切面高（腰+15）定标；加载失败 Empty 提示。**对齐/围度闭环/钳位不复位**：滑杆是裸权重，不经过 retired 链任何守卫。
