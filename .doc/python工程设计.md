...
4. 刀口（§4，法向投影同前片 flow 私有实现）：膝围×2、臀围×2（`back.hip_outseam_point`/`back.hip_inner_point`）、横裆线交点×2（**按净边链整链求交 `_h_cross_points`，勿预设载体边**）、浪尖、卷边起折×2、毗围点（thigh 录入时）、口袋对位（贴袋顶线 `back.patch_net_seg1` 自侧端沿袋口方向延长 ∩ 侧缝链 `_chain_hit`）、后中拼接 cb_top。d=0 时毗围线与横裆线共高，外缝刀口几何重合属固有（非 bug）。
