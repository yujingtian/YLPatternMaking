...
### 10.11 3D 人台显示（MakeHuman 下半身纯切割 target 滑杆试验场；试穿链 + 围度闭环 2026-09-12 退役）

**定位**：右栏 3D 主视图常驻人台（进系统即有）= **MakeHuman 下半身切割人台 + 官方 target 滑杆试验场**（2026-09-13 用户口径：**只显示下半身**、零姿势修改、形态调节用它自己的 targets）——base.bin = 纯切割链产物（A-pose 腿距原样、粗裁去臂、精裁腰+15、封盖水密；官方 12 场已按切割后索引重映射），六部位双极滑杆（腰/臀/大腿/膝/小腿/踝，−1..+1）直接驱动场权重，拖动实时 morph + 站点围度读数（站高 = vendor 地标检测值）。原生场固有缺陷（thigh 场膝上死区等）属作者化行为，原样呈现。**无自动化闭环**（围度闭环/纵向对齐/体型库已退役）：滑杆值就是场权重 w（作者化域 [0,1]，试验场放开到 ±1 供试探）。演进史（试穿布料/PBD/碰撞 → 体型调节闭环 → 静态 raw.obj 直显 → base.bin 姿势链滑杆 → 全身原生滑杆 → 纯切割 base.bin 滑杆六次转向 → 小腿/踝引入）见决策日志 §十一；恢复闭环/试穿 = 前端 revert、引擎零改动。
...
- **现行链路**：`Fitting3DView`（three 惰性分包；场景/格网/OrbitControls/视角预设/截图/显示开关；无 props、零 payload）→ `loadBodyMesh()`（bin.ts，模块级 promise 缓存）：fetch `targets.json` → 以其 `baseSha256` 指纹拉 `base.bin?v=<sha8>`（vendor 重跑后指纹必变即击穿浏览器缓存）→ `parseBaseBin` 二进制解析（布局头注 + 尾部残留/索引越界防爆）+ 与 targets.json 计数对账 → BufferGeometry 直显 → 滑杆 onChange：双极值映射场权重（正推 `site+`、负推 `site−`，两场独立作者化非反对称）→ `morphPositions`（稀疏叠加 pos = base + Σwᵢ·Δᵢ，签名只吃 `{positions, targets}` 结构）in-place 写 position attr + computeVertexNormals → `readGirth` 四站围度读数。初始机位按裁切面高（腰+15）定标；加载失败 Empty 提示。**对齐/围度闭环/钳位不复位**：滑杆是裸权重，不经过 retired 链任何守卫。
