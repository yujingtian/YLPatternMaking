...
- **二期留位（本次不建）**：会话/记忆/存储（agent/sessions.py 空位）、前端接线（webapp api.ts 不动）、流式/轮询/job 表、多轮照片追问。测试：tests/test_agent_api.py（TestClient + FakeVLM 全程离线，fastapi 缺失整文件跳过）；注入缝 = `monkeypatch(agent.runner, "_build_provider", lambda p: FakeVLM(...))`——真实路径恒返 None 交门面惰性构造。
