...
- **Read 显示可能与磁盘字节不一致（2026-09-19 前端 buildWaistRing 插槽 bug 三轮误导）**：TS 数组插槽错写（z 插值第二项磁盘上是 `src[b]`〔x 槽〕、应为 `src[b + 1]`）连续多轮 Read 显示均"看着正确"。数值定位法：插桩打印该行全部输入后**手算期望值 vs 存储值对照**（复算 10.115 vs 实测 8.263 即证执行的写入行非所见行）；失配的 Edit（磁盘确有该文本却 not found）本身就是显示/磁盘分歧的证据。显示层不可尽信，字节级核验（grep -n / cat -A 定位行）优先于反复重读。
...
3. 共享收尾 `_finish_piece`：主版坐标 **Y 轴反射**到局部（`local=(x−origin.x, origin.y−y)`：X 不翻保侧缝在左/前浪在右、Y 翻让腰头在上袋身向下，同 `piece_svg` 不翻转口径）→ **自定向**（反射 det=−1 翻转绕向，闭合多边形 shoelace > 0 则反转边序 + 每条 geom 反向，目标 < 0 保 cutter 外法向外扩，同机头负面积约定）→ 竖向丝缕线（经向 = 大片裤中线垂直方向 = 局部 Y，bbox 中心 x，上下各留 15%）→ 先缩水后缝边（§2.1）→ 刀口投影（§2.2，flow 私有 `_project_notches` 整体替换 gross_notches，同机头/前片先例不动 cutter 公开 API；`_finish_piece` 形参 `notch_dirs_main` 给出即袋贴切线方向投影、None 走贴袋角点投影）→ 装配 PatternPiece + 局部 ctx。**PatternPiece 登记 `origin=origin, frame="reflect_y"`**（2026-09-15 补：裁片要进 3D fitting payload 就必须带——漏登记序列化成局部系原样输出、被前端当全局坐标消费〔袋贴曾摆到脚底〕；SVG 走 bbox 定位不受影响，故此坑只在 payload 链暴露）。**坑：刀口延伸方向向量必须与刀口点走同一仿射链（Y 反射翻 dy → 缩水 dx/(1-weft) / dy/(1-warp)）——各向异性缩放会转动方向向量，直接复用主版切线方向再投影就不再是缩水后袋口弧线的切线**（交点在缩水->缝边后的毛样折线上以 `_ray_hit_poly` 射线求取取最近命中，无命中回退沿射线平移一个缝份/净点）。
...
3D 试穿 fitting 出口已程序化（`exporters/fitting.py` 的 `build_fitting_payload(ctx, m)`，纯 stdlib：整版全局系 cm Y-up 序列化前后片净边链/边名角色表/站点围度/腰头标量；`PatternPiece` 尾部新增 `origin/frame` 默认字段随片携带，pieces.py×2 + cutter.py 三处重建点透传，金标钉死）。双通道 `POST /api/draft/fitting` 与 `engine_glue._fitting` 同步（tests/test_engine_glue.py 全等）。金标：tests/test_fitting_payload.py。前端 2026-09-12 两次裁撤（①试穿布料/PBD/碰撞 ②体型围度调节/morph/闭环）后多次改向；**2026-09-13 试穿链二期复活：payload 消费恢复**（garment/ 纯函数层 + sim worker，schema v1 零改动，围度闭环部分仍退役），人台 = MakeHuman 下半身切割+站直 target 滑杆试验场（base.bin 站直姿势下半身切割网格 + 14 官方场〔12 measure + 身高 macro ±〕重映射后裸权重实时 morph，预设体型芯片 155/160/165/170 + 身高 cm 连续滑杆，fitting3d/ = Fitting3DView + bodymesh/{bin,morph,types,slice,height} + garment/）。**坐标系/人台口径权威 = §10.11**，演进见决策日志 §十一。

**裁切层与几何踩坑速查已独立为 §10.10（按主题分组）**；各口径的发现过程与纠偏轮次见 [决策日志.md](决策日志.md)。

### 10.7 Web 端（webapp/，一期 2026-08 落地）

主流程（2026-09-11 交互重构定型，此前「左全量参数 + 右 tabs + 顶部按钮工具条」平铺结构退役）：左栏参数分两页签（**核心参数** = 基础测量 8 + 款式开关组 + 腰头类型白名单平铺，**全部参数** = 原两段式分组面板原样）+ 底部固定动作条（引擎 Tag + 生成/导出/高级编辑）；右栏 **3D 人台试穿常驻主视图**（不再以整版已生成为前提）。「生成」只重算 3D 试穿；2D 整版/裁片**按需再算**——进高级编辑或导出时 `ensureSheet/ensurePieces` 自动补算（快照缺失或过期才重跑，新鲜直用）。**高级编辑** = 全屏 2D 工作台（整版拖拽调版 + 逐裁片预览 + 报表，退出回 3D；左栏保持可见，拖拽回写→参数高亮闭环不变）。**导出** = 导出中心弹层（勾选 7 类产物逐项串行下载）。二期拖拽调版（2026-08-25）：版上把手拖动 -> 反解参数 -> 回写参数面板并实时重生成（参数⇄几何双向绑定）。启动：`pip install -e ".[web]"` 后 `uvicorn webapp.backend.app:app`（前端 dist 已构建并由 FastAPI 托管；改前端走 `cd webapp/frontend && npm run dev`，Vite 代理 /api）。
...
- **交互重构落地形态（2026-09-11）**：Toolbar/PreviewPane 删除。左栏 = Segmented 页签（CoreParams 白名单平铺 / ParamPanel 原样）+ IssueStrip（错误/警告 Alert）+ action-bar；右栏 = Fitting3DView 常驻（删 800ms 自动重试穿与快捷滑杆卡——手动生成语义，保留首挂载自动生成一次；stale = 舞台 Tag + 生成按钮角标）或 AdvancedEditor（PreviewPane 改造：删 3D tab，进入自动 ensureSheet，收编撤销拖拽/裁片生成按钮）。叶子控件抽 `ParamInput.tsx`（ParamPanel/CoreParams 共用，含 pocket_type/fly_type 虚拟下拉与 custom_shape 编辑器）。**导出中心 ExportCenter**：整版/裁片 SVG 与报表走内存快照 `downloadBlob` 直存（本地引擎可出零后端，stale 先 ensure）；DXF/TOML 走后端重算（天然最新，不受快照新鲜度门控）；推板 DXF 未配置行内「去配置」；多文件 150ms 间隔串行（Chrome 多文件授权一次）；单项失败不中断队列、按项落账。useDraft：generate* 改 ref 取参（deps []）+ 返回快照本体；`download` 失败 setErrors 后**重抛**（导出队列按项落账，void 消费处须自带 catch）。
- **两步生成与下载门控（2026-08-25 用户口径，.claude/plans/webapp-phase3-two-step-generation.md）**：生成拆两步——`POST /api/draft/sheet`（整版 SVG+报表，不收集裁片）与 `POST /api/draft/pieces`（动态裁片清单+跳过说明；后端无状态，本端点自跑整版再提取）；旧 `/api/draft` 已移除（消费方仅前端+测试，CLI 不经 FastAPI）。先整版后裁片只是前端 UI 门控（先画后裁）：过期机制=参数版本号——任意参数修改 version+1，产物存 `{data, version}` 快照，版本不等即「已过期」（预览保留，提示=目标按钮右上角 StaleFlag 角标+Tooltip，2026-09-07 起弃预览区横幅；3D 试穿另有舞台 stale Tag）；**导出口径 2026-09-11 改「导出时自动补算」**——DXF/TOML 后端重算天然最新不受快照门控，SVG/报表源自内存快照、下载前 ensure 自动补算；裁片生成仍须整版已生成且未过期。互斥：sheetBusy/piecesBusy 互斥、单一 dlBusy 串行下载（DXF 下载重跑引擎）；localStorage 仍只存参数，刷新后生成状态清零。
...
- **回退阶梯（src/engine/client.ts）**：`?engine=off` 不建 worker（验收对比用）→ init 超时 60s 或 boot 失败会话降级（sessionStorage 'yl-engine-degraded' 防抖动重试）→ 单次超时/引擎异常本次回落并计数，连续 3 次会话降级 → worker 崩溃自动重建一次、再败降级。StrictMode 双挂载安全：模块级单例 + init 记忆化。UI 呈现：左栏底部 action-bar 角标「本地计算（绿）/服务端计算/引擎加载中」（useDraft 映射 client 的 unavailable→http）。
...
### 10.11 3D 人台显示（MakeHuman 下半身切割+站直 target 滑杆试验场 + 衣片展示〔十期现行：整裤+腰头缝合 旁挂自由垂/穿台碰撞落位 双视图 + 其余平铺〕）

**定位**：右栏 3D 主视图常驻人台（进系统即有）= **MakeHuman 下半身站直人台 + 官方 target 滑杆试验场**（2026-09-13 用户口径：**只显示下半身**、模特站直姿势、形态调节用它自己的 targets）——base.bin = 切割+站直姿势链产物（站直姿势：官方 rigs 骨架+蒙皮权重 clean-room LBS、大腿/小腿/脚逐关节角度、髋-膝-踝三点铅垂、脚底回水平，粗裁去臂、精裁腰+15、封盖水密；官方 14 场 = 12 measure〔腰/臀/大腿/膝/小腿/踝〕+ 身高 macro ±〔`macrodetails/height/` max/minheight〕已按切割后索引重映射 + 增量随帧旋转，身高场 ΔH 切割前实测写 meta.height），六部位双极滑杆（腰/臀/大腿/膝/小腿/踝，−1..+1）+ **预设体型芯片（155/160/165/170/默认）+ 身高滑杆（cm 连续、钳常人窗 150~185、可停任意身高如 162.3）**直接驱动场权重，拖动实时 morph + 站点围度读数（站高 = vendor 地标检测值 × 身高因子）。原生场固有缺陷（thigh 场膝上死区等）属作者化行为，原样呈现。**无自动化闭环**（围度闭环/纵向对齐/体型库已退役）：滑杆值就是场权重 w（作者化域 [0,1]，试验场放开到 ±1 供试探）。**衣片展示（重建一期静态直出 → 二期引力下垂 → 三期全裁片平铺+平面拼合 → 四期前身缝合立起 → 五期前身自由垂 → 六期后身缝合立起+拉直+顶部圆弧 → 八期整裤缝合 → 九期腰头缝合 → 十期穿台双视图，演进全录决策日志 §十一）十期现行 = **整裤+腰头单 sim 双视图〔旁挂自由垂悬挂 / 穿台人台环场碰撞落位+读数〕+ 其余平铺**：前片+袋贴并集宿主（garment/panel.ts buildFrontPanel）与后片+育克并集宿主（buildBackPanel：back top 段丢弃内部化、yoke 余边反向闭环接入，cb 反向段与 back.cb 同名相邻聚合成整条后浪裆尖→腰口贯通育克、yoke 腰口 top 顶替 top_chain；有省款省闭口错位守卫拦下退化纯后片、退化时 top 边角色升回 top_chain 供钉挂）合并为一条完整整裤四 part（assemble.buildFullPair，六期双筒 buildHangPair 退役）：四族缝合对（seams.ts——前中 rise/后中 cb 镜像族 + 侧缝/内缝跨宿主弧长族〔吃势均匀吸收〕+ 四裆尖 tip 补焊零 rest 闭环自动坍缩为裆交叉点）一次缝齐〔用户口径①侧缝缝合形成腰圆②内缝缝合形成两裤筒③四缝汇集裆交叉〕；解算**自由垂**（同日（五）用户口径「只保留腰部圆形撑开，其他地方真实物理垂挂」：腰口整圈全向钉 = 圆形撑环、其下布自重下垂出自然褶皱、两腿像真挂裤前后压扁垂落〔无布-布碰撞，两腿相贴处可能轻微互穿〕；芯碰撞机制保留——drape 传 field 即生效，仅当前展示口径不用）；**腰头立体化（九期现行）**：腰头片作第 5 参与片缝入腰圆（两端前中会合、中点后中）、挂腰头 = 腰口区全钉（身片腰口环 + 带顶/带底两缘，band.ts，口径详见下段 drape）；前片/袋贴/后片/育克作**贴层**（garment/rider.ts 重心绑定宿主三角形）逐帧回填渲染、袋贴径向内偏衬里侧（「前片在前口袋上面」口径的立体侧对应）、后片/育克径向 0 相邻非叠层；其余裁片（腰头）照旧按纸样净样原形行式平铺地面、逐片分色（与 2D 裁片 SVG 一比一，验证裁片形状出在哪层），见下「衣片链」条；引擎 payload schema v1 零改动、照旧含第 4 片 back_yoke〔back_yoke 开启时，无 yoke 向后兼容〕与第 5 片 front_facing——前端现消费全部片。演进史（试穿布料/PBD/碰撞 → 体型调节闭环 → 静态 raw.obj 直显 → base.bin 姿势链滑杆 → 全身原生滑杆 → 纯切割 base.bin 滑杆 → 小腿/踝引入 → 站直姿势 LBS → 试穿二期复活 → 整裤悬挂解耦 → 整裤链删除重建 → 三期回落全裁片平铺分色 + 机头后片/袋贴前片缝合 → 四期前身缝合立起 → 五期前身自由垂 → 六期后身缝合立起）见决策日志 §十一。

**已知问题（2026-09-17 发现，待修；非八期引入）**：fresh 浏览器 profile（无 localStorage）下自动首挂/本地计算 422——useDraft measurements 初始化 `saved?.measurements ?? {}` 为空，左栏表单显示的是 schema 默认值但 store 为空（首访用户踩、存量浏览器正常）；修法方向 = 初始化时把 schema 默认值提交进 store。

**引擎侧（exporters/fitting.py，纯 stdlib——2026-09-13 二期试穿复活消费，schema v1；2026-09-14 增育克第 4 片、2026-09-15 增袋贴第 5 片）**：`build_fitting_payload(ctx, m)` 自调 `build_front_piece`/`build_back_piece`/`build_waistband`（back_yoke 开启且机头已上版再调 `build_yoke`；front_pocket_facing 开启且袋贴已上版再调 `build_front_pocket` 取袋贴分支），输出 schema v1：`body{stations, points, crotch_drop, waistband_width, waistband_type, outseam}` + `pieces[front, back, waistband]`，有 yoke 时追加第 4 片 `back_yoke`（frame=`rot180`——育克整版倒置绘制，`local=(ox−x, oy−y)` 保向反变换；同时后片 top 边由 `top_chain` 改判 `seam`、育克 top 边顶替腰口链 `top_chain`；无 yoke 三片不变向后兼容），袋贴开启时再追加第 5 片 `front_facing`（frame=`reflect_y` 与前后片同口径反变换回全局系；外边 1:1 复制前大片——waist 腰弧段并入腰口链、side 外缝弧段缝育克侧边/后片侧缝上端、inner 接袋布 free）。**袋贴 marks = 整版袋口净线**（`collect_facing_marks` 公共导出、fitting 复用同一选择逻辑，2026-09-16 缩水不同态修复：piece.marks 随裁片 apply_shrinkage 同步缩放〔2D 出图正确〕，与未缩水 net_edges 两态混用——袋贴专用缩水款直接消费把缝合依据线放大错位 ~0.93cm〔用户尺寸单 front_pocket_shrinkage 0.1/0.06 实测〕，前端拼合守卫正确拦下但袋贴退独立行「分开」；金标 test_facing_marks_net_state_with_shrinkage 防回归）。`PatternPiece` 尾部两个 `origin/frame` 默认字段（裁片携带，front_pocket_flow 的 `_finish_piece` 已登记——漏登记会被前端当局部系原样消费）与三个重建点透传（`pieces.py` + `cutter.py`）一并保留。
...
- **边角色表**（引擎现行输出；消费方 = garment/mesh.ts EdgeRun 聚合段——缝合配对虽删，角色表与 mesh 层聚合保留供逐片重建）：front `waist:top_chain, hem:hem, mouth/fly_*:free, rise/inseam/side:seam`；back `waist:top_chain, hem:hem, cb/inseam/side:seam`（有 yoke 时 back 上边换名 `top:seam`〔机头下口线〕、腰口件改由育克承担）；yoke `top:top_chain, bottom:seam, cb/side:seam`；front_facing `waist:top_chain, side:seam, inner:free`。`girth_finished` 均为成衣量（waist/hip 整圈、thigh/knee/hem 单腿）。
- **双通道**：`POST /api/draft/fitting`（app.py）与 `engine_glue._fitting`/`_COMMANDS["fitting"]` 逐字段复刻，`test_engine_glue.py` 钉全等（前端 api.ts 引擎优先回落 HTTP）。改引擎后照常 `npm run build:engine`。

**前端（webapp/frontend/src/fitting3d/，Fitting3DView + bodymesh/{bin,morph,types,slice,height} + garment/（衣片静态装配），依赖 three；garment 另加 delaunator）**：
- **现行链路**：`Fitting3DView`（three 惰性分包；场景/格网/OrbitControls/视角预设/截图/显示三态开关；收 fitting 快照 props 喂试穿）→ `loadBodyMesh()`（bin.ts，模块级 promise 缓存）：fetch `targets.json`（**`cache:'no-cache'` 带 ETag 再验证——它是击穿链的链头，裸 fetch 会被浏览器启发式缓存钉死旧指纹、指向旧 base.bin，vendor 重跑后整链失效**）→ 以其 `baseSha256` 指纹拉 `base.bin?v=<sha8>`（vendor 重跑后指纹必变即击穿浏览器缓存）→ `parseBaseBin` 二进制解析（布局头注 + 尾部残留/索引越界防爆）+ 与 targets.json 计数对账 → BufferGeometry 直显 → 滑杆 onChange：双极值映射场权重（正推 `site+`、负推 `site−`，两场独立作者化非反对称；身高同构——heightW 正推 `height+`/负推 `height-`，**预设芯片 = `weightFor` 按 meta.height 实测 ΔH 把目标 cm 换算成权重一键设值（155~170 落 w=−0.34~+0.04），身高滑杆 cm 值同一换算反向驱动**；换算纯函数 height.ts：heightCm/weightFor 互逆 + ±1 钳制 + stationFactor）→ `morphPositions`（稀疏叠加 pos = base + Σwᵢ·Δᵢ，签名只吃 `{positions, targets}` 结构）in-place 写 position attr + computeVertexNormals → `readGirth` 六站围度读数（**站 y × stationFactor = heightCm/baseCm 身高因子——身高场近似等比升降全身，围度站随身体移动；比例近似口径，与「场会微移站高」caveat 同级**）。初始机位按裁切面高（腰+15）定标；加载失败 Empty 提示。**对齐/围度闭环/钳位不复位**：滑杆是裸权重，不经过 retired 链任何守卫。
- **衣片链（九期现行 = 整裤+腰头缝合自由垂悬挂 + 其余平铺；演进见决策日志 §十一）**：useDraft.generateFitting（protocol `'fitting'` cmd 引擎优先回落 HTTP，payload 照旧全片）→ 前端消费全部片，两路分流——**前后身并集宿主 → 整裤立体链**（`panel.buildFrontPanel` / `panel.buildBackPanel`，见下段）+ **其余裁片平铺** `assemble.buildFlatLayout(payload, exclude)`（exclude = 前身组锚 front_piece〔panel.hasFacing 再加 front_facing〕+ 后身组锚 back_piece〔backPanel.hasYoke 再加 back_yoke〕：排除锚且贴合守卫通过 → **整组**离开平铺〔缝合整体要么全平铺要么全立起〕，守卫不过组员照旧独立行 = panel 退化兜底；内部逐片 `garment/mesh.buildClothMesh`〔边界弧长重采样+错排栅格+delaunator 质心过滤；内部三角形最小角 >15°、面积比 ≥99%，边界细长三角形原样；距离/弯曲约束与边界环字段为解算重建预留〕后**行式平铺** y=0 地面——payload 逐片一行〔行序 = 片序、行距 FLAT_PRIOR.rowGap(6)〕；**缝合拼合组 STITCH_GROUPS**（结构化：members + **贴合守卫 aligned**——缝合依据线双向最近点距 max <STITCH_GAP(0.5cm)〔seamGap，edgePts 按边名取链 / facing 用 marks〕才拼合，不贴合或组员缺片退化独立行，**不错位硬拼**）〔晚八 back+yoke「先将机头和后片缝合成为一个整体」→ 晚九 front+facing「将前口袋也平铺出来，然后和前片缝合，形成一个整体」〕：组内成员按**整版全局坐标**对齐（共享组 bbox 归一变换）= 缝后位置天然贴合——back_piece+back_yoke：back 上边 top = yoke 下边 bottom（整版同一净线，无省款 fixture 实测逐点重合 0；**有省款 yoke 下边是省闭口净样、与整版下口线错位实测 ~12cm = 后片省未收的真实缝前状态，守卫拦下退独立行**，处理留后）；front_piece+front_facing：缝合依据 = **facing 袋口净线（marks，任一条）↔ front mouth 边**——直/弯腰头两款 fixture 实测 marks[0] 与 mouth 逐点重合 0（facing 外边 1:1 复制挖削**前**大片轮廓、按整版坐标摆位自动补齐袋口月牙缺口，袋口净线即缝合线）；**组内叠层**：第 j 片 y = j×FLAT_PRIOR.stackStep(0.2)——袋贴袋口条带与前片净样重叠，共面 z-fight 微抬分层；**方向口径（2026-09-16 用户拍板「前片在前口袋上面」）**：members = [front_facing, front_piece]，facing 片序在前 = 下层、前片叠上层——俯视袋口条带区显示前片蓝色（外观主体是前片，袋贴衬里侧）〕；成对裁片〔front/back+yoke 拼合组/facing 及后续加片〕L 原样归一 (u, 0, 行基 z+v) + R **整组**镜像 u→W−u 平移 L 右侧留 FLAT_PRIOR.gap(4)〔拼合组关于组中线镜像——真裁剪 yoke 也是 L/R 两片〕、腰头单片〔SINGLE_PIECES 整根幅样无左右之分、side 固定 'L'〕，全组 z 居中〔组中心 z=0〕；bbox 由网格顶点实算不信任 payload 标量；无弯折无解算、网格即净样原形，与 2D 裁片 SVG 一比一；**验证分流**：平铺即错 = payload 边链/网格层问题〔引擎 build_fitting_payload 或 mesh.ts 上游〕，平铺对悬挂错 = 摆位/解算层问题）→ `render.buildGarmentView`（**逐片分色** PIECE_COLORS：front 蓝/back 绿/waistband 橙/back_yoke 紫/front_facing 青、未登记片灰兜底；每片一 Mesh 一材质，`pieceColor(key)` 供 UI 图例同源取色；Fitting3DView 侧栏「裁片」卡图例 = **固定片序**〔PIECE_LEGEND_ORDER：前/后/腰头/育克/袋贴〕色点 + 中文名，payload 没有的片灰显「未开启」+ hint 标开启路径——晚九后排障：用户报「前口袋没渲染」实为 front_pocket/front_pocket_facing 默认关、payload 无第 5 片非渲染 bug，图例固定序让缺片一眼可见〔2026-09-16 起 Web 初始参数**产品层默认开**口袋族（useDraft 初始化对 localStorage 缺键补 front_pocket/front_pocket_facing = true——「标准牛仔裤」口径，引擎 PatternOptions 默认保持最小裸版：extract 探针守卫拒绝口袋特征 + 部分 extract 测量组合 facing 默认参数几何越界，改引擎默认当日回滚，详见决策日志 2026-09-16 条）；灰显 = 用户显式关过对应开关〕）；显示层平铺 Group `position.x = hangX + envelopeR + FLAT_PRIOR.clearance(6)`（让位到整裤悬挂右侧；hangX/envelopeR 见下段立体链；morph 增量由净空兜底）、`position.y = FLAT_PRIOR.lift(2)`；hangXRef 存**全场景包络中心**（人台左沿 ~ 平铺组右沿的中点——双主体 = 人台+悬挂筒 ↔ 平铺组），相机 target/机位按场景中心差量平移居中〔幂等〕，flyTo 四预设同口径。**前后身并集宿主立体链（八期现行接线）——panel 并集宿主 + rider 贴层 + 整裤 drape 自由垂**：`garment/panel.buildFrontPanel(payload)`（**前身并集宿主**：front_piece 边链 mouth 段〔free〕原位替换为袋贴月牙边——mouth 两端点投影到 facing 的 waist/side 边取部分段，waist 部分段命名 `'waist'`/top_chain、side 部分段命名 `'side'`/seam，与 front 同名相邻边 **mesh runs 自动聚合** → drape 悬挂 pin 语义自动覆盖袋贴腰口段〔整个前身挂腰，物理正确〕；**几何依据**：facing 外边 1:1 复制挖削**前**大片轮廓，袋口月牙区在 front 净样**之外**——facing 不能直接绑 front 网格〔月牙区顶点 locate 全 null〕，必须并集；守卫 = `assemble.frontFacingAligned`（与平铺拼合同口径提炼共用：marks 与 mouth seamGap <STITCH_GAP）+ mouth 端点投影 <JOIN_TOL(0.5cm)，失败退化纯前片宿主 + 袋贴留平铺〔warning 上屏提示〕）与 `garment/panel.buildBackPanel(payload)`（**后身并集宿主（六期）**：back 边链 top 段〔机头下口线 P0→PN，有 yoke 时角色 seam〕**丢弃内部化为缝线**，数组末尾接 yoke 余边反向闭环路径〔cb P0→O + top O→X + side X→PN，reversedEdge 端点对调〕——**重排聚合关键**：back.cb〔裆尖→P0〕与 yoke cb 反向段〔P0→O〕同名相邻，runs 聚合成**整条后浪**（裆尖→腰口贯通育克，yoke fixture 29.0cm）→ drape 后中缝合对沿 cb 配对天然缝到腰；yoke 腰口 top 反向段顶替 top_chain〔悬挂 pin 自动覆盖〕、yoke side 反向段与 back side 同名接续；守卫 = `assemble.backYokeAligned`（与平铺拼合同口径提炼共用：back.top ↔ yoke.bottom seamGap <STITCH_GAP，无省款 fixture 逐点重合 0）失败退化纯后片宿主 + 育克留平铺；**无育克 = 正常款型**纯后片即后身、信息性 warning 非守卫失败）→ `garment/rider.bindRider/rideRider`（**贴层**：前片/袋贴不进粒子系，每顶点 host.locate 重心绑三角形〔重心权重对仿射变换精确，verlet 帧间分段线性零额外误差〕，L/R 共享一份 2D 绑定〔hostOffset 0 / hostN 区分两侧〕；**locate 缺口兜底 = 宿主边界环最近段插值**〔Delaunay 无约束+质心过滤在凹弧丢窄楔形三角形——月牙外边恰是凹弧，实测 facing 11/76 顶点命中；段插值 = 弦近似弧、误差亚毫米，远优于最近角〕；`rideRider(b, hostPos, hostOffset, out, radialOffset)` 重心重建后按 `(r+radialOffset)/r` 径向缩放 px/pz〔y 不动〕——袋贴 −HANG_PRIOR.riderStep(0.2) 衬里侧）→ `render.buildRiderView`（每 side 一 Mesh、逐帧回填 + computeVertexNormals，材质口径同 buildGarmentView、PIECE_COLORS 逐片分色）→ `garment/core.buildCore`（**纸样围度撑型芯 v3 两腿分离三实体**〔八期重写〕：躯干管〔fork+TORSO_BOTTOM(2)→腰+8，半径 = rFork〔毗围代理 max(臀,2×腿) 防掐腰〕→rHip→rWaist 插值；底部 CROTCH_FILLET(6) 带 = **裆圆角**——截面 z 向压缩成椭圆（zScale 底行 CROTCH_ZMIN(0.28) smoothstep 到全圆、x 保持全宽）：真身形骨盆底耻骨/会阴区前后收拢左右髋仍宽，无圆角的平底圆柱会把前/后中布撑到全半径壁再收回裆点 = 裆上生硬尖帐篷（八期当日用户报障，前中缝终态 z 3→16→12 实测；圆角后单调顺滑卷入）；fork..fork+2 带仍是腿管口袋〕+ 左右分离腿管〔hem−1.5→fork+4 圆环，半径 = 腿剖面 装站→thigh→膝→hem 逐站 coreRadius(g)=g/2π−CORE_SKIN(0.98)，轴 |x| = r+gapHalf(y)，gapHalf 从 fork 处 LEG_GAP_MIN(0.8，防双管相切刀口)线性张开到 LEG_GAP_TAPER(12) 以下 LEG_GAP_HALF(2.6，腿间总隙 5.2)——**裆下两腿真分离**，腿间空隙给内缝焊线与裆交叉点安身；腿管上延过 fork：fork 带截面只有分离腿管，躯干环不出现在 fork 行（否则裆尖摆位困进管内）；各实体独立水密（tube 行环+条带+端盖，腿管锥形轴两端盖心随行）；v1 四死路对策与 v2 花生证伪详见 core.ts 头注与决策日志八期条——**花生腰谷在径向场下仍是实心桥，内缝焊不上**，径向场表示不了腿间空隙〕；`buildLegAxis(payload)` 导出 rAt/cAt/forkY 供摆位）→ `placement.buildBodyField`（支撑半径场 R(y,θ) 凸上界查表〔摆位半径口径〕**+ 行截面环留存**〔SliceRing：躯干行 1 环/腿行 2 环，质心+包围半径 quick reject——drape 截面碰撞口径；撕裂碎片环不进碰撞〕）→ `assemble.buildFullPair(front宿主, back宿主, field, forkY, legAxis, band?, lift)`（八期整裤四 part 摆位：parts=[fL,fR,bL,bR] offset 0/nF/2nF/2nF+nB；**①片身**——躯干段〔y≥fork+forkBlend〕placePoint 全局 x→θ，腿段**腿局部圆环绕管**：逐高度局部归一 t=(x−xi)/(xo−xi)〔xi/xo=inseam 链/side 合链 chainXAt 折线插值——牛仔裤侧缝/内缝撇势弯曲由逐高度取链 x 吸收〕，φ=t·π 从腿内侧经前/后到腿外侧绕腿轴 ±cAt(y) 铺 rAt+skin+gap，内侧线钳 ≥0.3 不越中线；fork 向上 forkBlend(8) 线性过渡〔裆角布从躯干前面顺滑收进腿间；混合弦线落环内回退径向摆位保初摆位零穿透〕；**②腰圆 360° 整圈弧长重参数化**——顶链按腰口弧长分数 f∈[0,1]〔order[0]=中缝腰角〕铺 fL −f·90°/fR +f·90°/bL −180°+f·90°/bR 180°−f·90°：中缝腰角镜像共点 0°/180°、侧缝腰角共点 ±90°，**腰圆初始已闭**（口径①的摆位侧前提）；**③侧缝语义摆位竖直**——躯干行径向场 ±90°、腿行 = 腿外侧线 cAt+rAt+skin+gap，fork 带内线性混合（'side' 全 run 覆盖，后宿主育克侧段+后片侧缝各 run 同顶点集）；**④内缝语义摆位 = 腿内侧线**（cAt−rAt−skin−gap 钳 ≥0.3）——**前后宿主相邻共线，内缝焊对初始间隙 ≈0**（口径②），L/R 腿内侧线隔腿间隙相望不交叉；**叉口守卫**（内缝顶段 y>fork 跳过本步、归 step1 躯干/混合摆位——穿台体叉口可低于纸样裆深〔实测体叉 70 vs 纸样裆 78〕，钳位内侧线 x=±0.3 会插进骨盆环；旁挂纸样叉 = 内缝链顶，严格 > 不触发、旁挂回归零改动）；**侧缝腰角前后共点 snap + 高差渐变**（前后腰口曲线起翘差实测 front 97.7/back 98.1、退化款 ~4cm——角点本体 = side 合链首采样，snap 在侧缝摆位之后写，否则钉与缝在腰角永久拔河；（十）角点两侧 waistBlendSpan(4cm) 弧内顶链 y 向 snap 高度渐变〔只 snap 角点会在缝口两侧留台阶、腰头底边跟出折点；渐变须按新 y 重算半径——不更新会陷碰撞壳 ~0.07cm 与 collide 打架，钉恒守金标实测抓住〕）；**⑤统一抬升 lift（参数化）**〔末段统一施加含全部钉目标；旁挂 = hangLift(12)——整裤全长 ~99cm 自由垂拉伸+褶皱收拢余量，下摆终态离地 3~4cm 不拖地；穿台 = anchorLift 腰地标锚（钉初始即钉在人台腰地标高度，落位下放由 settle 控制器接管，见下「穿台链」）〕）→ `seams.buildSeamSet(garment)`（**四族缝合对纯函数**：rise/cb 镜像族〔同宿主 L/R 同号顶点，fly 连裁 rise 缺失整族跳过〕+ side/inseam 弧长族〔驱动链=front 逐顶点 → 对侧 runIndexAt 弧长分数；side 锚腰口端/inseam 锚脚口端；back 宿主 side 双 run 先 mesh.mergeRuns〔首采样 y 降序〕合链；吃势前后链弧长差沿链均匀吸收〕+ tip 补焊 ×2〔tip 本体 = rise/cb 链首采样——角点共享规则下 inseam 末采样是 tip 前一步；fly 兜底 = inseam 末采样环下一点〕；rise 末对+tipL+cb 首对+tipR 零 rest 四边形自动坍缩 = 裆交叉点〔口径③〕；SeamSet{pairs 扁平 2S rest=0 + groups 分族切片}——drape 求解循环对全族一视同仁零改动）→ `render.buildGarmentView`（每片一 Mesh 共享白坯材质 DoubleSide）→ `drape.buildDrape/stepDrape`（主线程轻量 verlet，现行口径 = **整裤+腰头自由垂解算**〔九期挂腰头：有 waistband part 时**腰口区全钉**——身片腰口环整圈 + 带顶/带底两缘（带 = 两缘被持的真实布、中间布自由；只钉带顶会被自由垂挂对折趋势把前后中往里拖 1.2cm 开楔形洞、加钉带底仍留 GS 冲突平衡 ~0.5cm 均匀缝口——软传力零缝口必须一侧硬钉）、sideHold 退役、接缝孪生角 bandWaist 补配对；无腰头回退（五）整圈钉〕〔（五）→（九）混合形态→（十一）回归**全域自由垂**现行（用户口径「腰头一圈+下面真实物理悬挂+侧缝不出山脊」）：buildDrape 传 null；（九）的 D 形不对称诊断系度量 bug——正确度量证实自由垂截面前后基本对称（前 +11.2/后 −9.8），山脊=缝尖折痕，由**宽摊平窗** seamFlattenSpan(5cm) 处理（缝条像熨过的缝头硬条、折痕移窗缘、缝身平顺；±3 不够——缝仍是折尖）；（九）collideAboveY 机制保留备用（传 field+下限即混合形态）；腿筒前后压扁、两腿相贴处可能轻微互穿〔无布-布碰撞〕；下述芯碰撞机制保留、drape 传 field 即生效，当前口径不用〕：**pin = 腰口整圈全向钉**（四 part 360° 腰圆支点，顶链全量+终点角+sideHold(10) 刚度带〔腰头代形——本期不含腰头，真实成衣此区由腰头撑圆〕；「钉与缝同意」由腰圆重参数化+侧腰角 snap 按构造保证）+ 约束 = mesh dist/bend + **全族缝合对**（seams.buildSeamSet 的扁平 pairs，rest=0 刚度 1，GS 迭代内跨片投影；**缝口摊平窗**（（八）用户报障「侧缝缝合的线有明显凸起」：自由垂挂前后片平行贴合、侧缝成 180° 刀锋卷边——距 side/inseam 链纸样 5cm 窗内弯曲刚度升 0.5〔mesh.bendKArr 逐约束覆写、drape.stampStiffBands 三窗：缝口摊平/腰臀过渡/脚口环带〕摊成缓坡；全局提 0.45 无效〔重力压平镜头截面、缝尖 r≈C/2=自然摊平宽度不动〕）；实测 rise/cb/inseam/tip 全族 avg=p95=0、side avg 0.026/p95 0.176）+ **撑型芯截面碰撞（八期开，替代五~七期自由垂的 field=null）**：collide 按 pos.y−yLift 取行截面环，粒子在环内或皮肤壳内〔到最近边界 <CORE_SKIN〕→ 推到最近边界+skin·**外法线**（内外两侧统一沿外法线——按「边界→粒子」方向推会把环内粒子越推越深自陷，实测修前 80 粒悬环内）+ prev 混合摩擦 0.8；深穿兜底只在包围圆内才做射线判内（pointInRings，v1 细龙骨穿膛守门）；性能口径 sqrt 代 hypot；crotchHold(0) = 裆尖短时硬钉 fallback 旋钮默认关（零 rest 闭环拓扑自动坍缩，无需旧链 crotchHold 60）；地面碰撞安全网；重力 −980；rAF 一帧一步至 settle，三款夹具+退化款实测 99~174 帧真收敛。yLift = lift 传入（collide 场查询回纸样空间，与摆位侧一致——穿台即 anchorLift）。seamStatsByGroup 分族统计金标用）（形态沿革——一期裆下折槽/二期消解、前中敞口→缝合闭合、fly 连裁 rise 缺失跳过缝合〔敞口属连裁门襟固有形态〕——详见决策日志 §十一对应条目。）**显示层偏移（旁挂不套轴，八期单整裤组）**：四个 rider 视图 Group 同点 `position.x = 人台基础网格半宽 + HANG_PRIOR.clearance(6) + envelopeR`、`position.y = 0`（envelopeR = field.maxRadius + garmentGap；独立原则——CORE_SKIN 只把芯半径锚**纸样围度**〔g/2π−0.98〕，与人台围度无约束关系，套人台轴必穿体；sim 空间已含 hangLift〔下摆离地 3~4cm〕显示层不再抬）；**初始摆位直接可见，随即 rAF 引力自由垂动画至静止**；相机 target/机位按场景中心差量平移居中双主体〔幂等〕，flyTo 四预设同偏移。**独立原则（用户拍板）不变**：人台滑杆不改衣服、打版参数不改人台；交互 = 重新生成（重发 payload）+ 自动首次生成 + fittingStale 提示；**绝无自动调版/自动改体型闭环**。金标 `panel.test.ts`（前身：mouth 内部化〔替换后无 mouth 边〕/waist 聚合变长〔袋贴腰口段并入 pin 域〕/**覆盖性 = locate 非空或贴宿主边界 <0.1cm**〔并集完备性的正确验收——边界三角形覆盖天然有缺口，locate 非 null 过强〕/面积比 ≥99%/两路退化〔无 mouth、守卫破坏——**测试构造平移 facing 必须 marks 连同 edges 一起平移**，marks 是独立字段只动 edges 守卫照过〕）/`rider.test.ts`（平面恒等分层容差：非兜底 1e-4 / 兜底 = 段插值弓高 <0.1；刚体跟随仿射封闭；径向内偏〔大半径柱面构造验证「半径 −0.2、y 不变」〕；兜底占比 front <5% / facing <25%〔凹弧边界点实测 11/76〕）/`garment.test.ts`（mesh 拓扑/场与摆位/穿透口径）/`core.test.ts`（v3 三实体：waist/hip 行恰 1 环+fork/knee/hem 行恰 2 环〔腿管上延〕/腿管周长逐站锁定围度/腿间隙 2×LEG_GAP_MIN→2×LEG_GAP_HALF/裆站不掐腰）/`assemble.test.ts`（三期全裁片平铺金标：parts 展开 序〔成对片 L+R、腰头单片〕/全片贴地 y≡0 + 全组 z 居中/行序 = payload 片序 + 行距恰 rowGap/成对片逐点等距变换 L 原样 R 前中镜像/腰头单片原样/+ 缝合拼合金标〔fixture_fitting_yoke.json 从 git d784de1 恢复（schema v1 未变直接兼容）：拼合行 parts 序 yoke 不独立成行/back top 边与 yoke bottom 边逐点贴合 <1e-4cm（L/R 两侧）/整组镜像/拼合行总高 = y 并集〕+ 袋贴拼合/守卫金标〔fixture_fitting_pocket.json **新生成**（引擎 FlowRunner(front_pocket+front_pocket_facing) 直出直腰头口袋款，补 ok/warnings）+ fixture_fitting_curved_pocket.json 从 git d784de1 恢复（弯腰头+有省 yoke）：拼合 parts 序/缝合依据数据前提（marks↔mouth 双向 <0.5）/**组共享变换断言**（L 侧 pos = xy − 常数 Cx/Cy 逐点恒定、R 侧翻号——「整版对齐」的本质断言，比摆位后测边距更强）/组内叠层 y（front 0、facing = stackStep）；守卫：有省 yoke 错位 ~12cm 拦下退独立行、弯腰头袋贴照常拼合〕——**坑两个：`Garment.pos` 是 Float32 渲染 buffer 口径，对照 Float64 `xy` 有 ~5e-7cm 量化伪差，toBeCloseTo 精度放小数 5 位（9 位必炸）；buildFlatLayout 内部自建 mesh，测试侧再调 buildClothMesh 是引用不同的新对象，toBe 引用断言必炸——内容一致性由逐点等距变换用例钉死，结构对账用顶点数**；悬挂金标保留：L+R 镜像对称 z 差 <0.1cm〔撑型芯场 θ-bin 量化伪差，人台场旧标 0.05〕/前扇区角度域/穿透按构造为 0〔查验回纸样空间——hangLift 抬升后 pos y ≠ 场查询 y〕+ 侧缝语义摆位金标〔side run 全顶点 θ=±90° 竖直、y = 纸样高 + hangLift；全 pos y ≥ hangLift〕）/`seams.test.ts`（纯拓扑：mergeRuns 合链〔yoke 款 back side 双 run 合链长度/arc 单调/腰口端优先〕/四族配对数 = 驱动链顶点数/锚定端〔side 首对 = 两链腰口端、inseam 首对 = 两链脚口端〕/tip 顶点号 = rise/cb 链首采样/fly 分支〔删 rise 边合成款：镜像族跳过、tip 走兜底〕/有省退化全族照跑）/`drape.test.ts`（八期换代四 describe = 基础/袋贴/育克/有省退化，共用 assertFullPants：600 帧内真收敛非封顶 + 无 NaN/四 part 整圈钉恒守初始位/**四族缝分族 stats**〔rise/cb avg<0.1 p95<0.3、side avg<0.15 p95<0.5、inseam avg<0.3 p95<0.8——实测全族 0 除 side 0.026/0.176〕/**裆四尖两两最大距 <1cm**（口径③量化）/side 首对〔两侧腰角 snap 共点〕<0.5cm/**不拖地 minY>1**〔hangLift 8 重定标〕/**终态截面穿透零**〔pointInRings 环口径——腿间空隙是合法布位，径向口径会误判〕；有省退化款 degradedSide 放宽〔side avg<1.0/p95<5.0：back 缺育克矮 ~4cm 的固有开口，panelHint 警示〕）；drape.test.ts 另有 **脚口环带刚度 stamp describe**〔2026-09-19（二）：hem 窗覆盖约束刚度 = hemBandStiffness 精确值、三窗外回落 bendStiffness（Float32 量化——0.3 存不精确须 toBeCloseTo 6 位）、规模把门 inHem>100/hemOnly>10/plain>50；腰头无 hem 链不 stamp〕）；assemble.test.ts 另增 **buildFullPair 六用例**〔四 part 结构/offset/腰圆四角精确落角+θ 沿链单调+侧腰角前后共点 <1e-3/内缝 = 腿内侧线〔z=0、|x|=medial〕/腿区圆环距离 ≈ rAt+skin+gap/hem 内角落内侧线/侧缝语义+镜像中位数+截面穿透零〕、exclude 四用例〔排除锚整组离开/只排锚组员随走/无袋贴款无组员/守卫破坏组员照旧独立行——**期望行序记 payload 片序，front_facing 在 back 之后**〕+ 后身组 exclude 三用例；六期双筒悬挂金标〔前/后片 L+R 扇区摆位/侧缝语义/穿透〕随 buildHangPair 退役删除），vitest node 直跑。**整裤缝合解算链（2026-09-13 复活版）已于 2026-09-15 整体删除**：seams/band/ease/heatmap/align/useGarment、worker/{entry,protocol,solver,collide}、pbd/constraints、yoke 与口袋变体 fixture 及其金标（solver/hang/ease 测试）、three-mesh-bvh 依赖；yoke 16 缝不收敛（全局摆振泵）的攻坚史、调参红线（collisionSkin 0.98 持续接触锚、丹宁增硬 0.45~0.5 助收敛、preRelax 300、crotchHold 60 硬释放、band rest 全保纸样矩形等）与已证伪方案（无碰撞单帧塌片 wrapHem 0.50、全钉腰头卡死、摩擦/阻尼/迭代单 knob 全反证）录决策日志 §十一 2026-09-14~15 条——**重建解算时先读**。
- **穿台链（十期，2026-09-18；侧栏 Segmented「旁挂|穿台」双视图切换 + 穿台「重新试穿」——旁挂分支原样不动、往返幂等；穿台 = 整裤真实穿在人台轴上，整裤组位 (0,0,0) 不偏移）**：碰撞体 = **人台切片环场**（四口径用户拍板之一，其余三条：下降 = 单次仿真内缓释钉高 / 起始腰位 = 人台腰地标 / 旁挂保留双视图）。**锚定**：θ 两系同构（人台 +Z=前、Y-up 脚底 0 ↔ 纸样系 (r·sinθ, y, r·cosθ)）→ 穿台只需 Y 平移 1:1 cm 不缩放：`anchorLift = landmarks.waist × stationFactor(heightW) − 纸样 stations.waist.y`（fixture 实测 7.644 = 人台腰地标 105.644 − 纸样腰站 98；身高 morph 的地标漂移由 stationFactor 比例近似吸收，与围度读数同口径同量级误差）→ `buildBodyField(shiftPositionsY(morphPositions(a, w), −anchorLift), a.indices, fieldYMin)`（当前滑杆权重 morph 后的人台网格平移回纸样系建场，场行 = 纸样系高度；**yMin 场域下探〔脚碰撞修复现行口径〕**：锚定下移后脚成纸样负 y〔世界 y∈[0, anchorLift)〕，行表从 0 起建则脚背以下无碰撞环 = 裤脚穿脚无推出〔diag 实测 56 粒子穿脚〕——`fieldYMin = floor(−anchorLift/rowStep)·rowStep` 对齐格点罩住脚底，行号换算 (y−yMin)/rowStep；BodyField 第 6 参 yMin 缺省 0 = 旁挂老口径字节等价）→ `placement.buildLegAxisFromRings(field, 裆地标×sf − anchorLift, 踝地标×sf − anchorLift)`（**人台切片环版腿轴**，第三参 ankleY 止踝——yMin 下探后负 y 行进来的是脚环〔包围半径含脚全长 ~18、前伸 cz 13+〕，混进 rProf 会让裤脚摆位喇叭张开〔r+|cz| 放大口径，合成场实测 rAt(0) 2.04→31〕，扫描止于踝行；旁挂仍用 core.buildLegAxis 纸样站口径：自裆地标向下 forkSearch(6) 窗找**首个恰 2 闭环行** = 有效叉口——体裆地标处切片可能仍单环，找不到 throw 由视图层转 hint；双环行 rAt = max(环包围半径 + |质心 z|)——LegAxis 轴心 z=0 近似，真腿环质心 z 偏移〔大腿偏前 ~2cm 实测〕会让绕管摆位前侧浅穿，包围圆按 (cx,0) 圆心放大 |cz| 保证环 ⊂ 圆(轴心, rAt)；cAt = |质心 x| 均值；非 2 环行跳过、相邻桥接）→ `buildFullPair(前宿主, 后宿主, 人台场, forkY = 腿轴叉口, 腿轴, band, anchorLift, waistRing〔2026-09-19 起第 8 参，见下「腰圈钉环」条〕)` → `buildDrape(pair, 人台场, anchorLift)`（碰撞查询按 pos.y − yLift 回纸样空间查行环，机制与旁挂同源、drape 求解结构零改动）→ `garment/settle.ts` **落位控制器**接管钉高。**落位状态机**（真人穿裤行为模型「拉到腰 → 裆不舒服一点点往下 → 裆舒服即停」的仿真翻译：向上唯一硬止点 = 裆〔浪长不可伸长〕、向下无几何卡点〔人台刚体无摩擦，支撑问题换成钉扎问题〕→ 最终穿着高度 = min(锚定腰位, 裆零穿透最高位)，单程搜索、掉裆是观感读数非力学量；前后裆独立探针 = 俯仰自然涌现）：`hold`（holdFrames 240 不动钉持锚松弛——启动序列红线：先让布静止再动钉）→ `lowering`（每帧裆探针：前 = rise 链自裆尖 probeSpan(4) 弧内采样 + inseam 链自脚口端回溯同窗 + 裆尖本体〔fly 连裁 rise 缺失兜底 tipAfterInseam〕、后 = cb 链同构；接触量 v = 环内 ? skin+d : skin−d 与 collide 平衡位同口径〔v=0 恰接触〕；穿透 > deadZone(0.3) 且未到 maxDrop(15) → 该侧 h −= dropRate(0.08cm/帧，准静态），写回 `pinTarget.y = baseY + lerp(hF, hB, f)`、X/Z 钉不动〔全钉保 XZ 刚度——穿台收敛战红线：Y-only 钉锁不住环向滑移模态〕；frontness f = |atan2(x,z)|/π〔0 前中 → 0.5 侧 → 1 后中，钉集全在同一腰圆上、位置即方位统一成立〕——**前后腰独立下放、俯仰涌现**；到 maxDrop 仍穿 = 单侧 fault 停放〔偏小读数路径，不与碰撞无限拔河〕；双侧入区或 fault 连续 confirmFrames(30) → settle）→ `settle`（钉冻结纯跑，**停走只认本相位全粒子真实静止**——下放瞬态 vs 泵的判据红线）→ `done` 出读数；下放中钉速被全粒子 avgSpeed 均值稀释出的假 settled 由 `wake()` 对治（settled/settledFrames/capped 清零 + maxFrames 续预算 settleMaxFrames 480）；maxTotalFrames(1800) 控制器硬顶 capped 兜底。**读数语义（独立原则：试穿结果是读数，不是自动调版闭环）**：DressReport = 掉裆 前/后（相对人台腰地标，正 = 掉）+ 前/后裆接触三态（穿透 X.X cm / 贴合 / 间隙 X.X cm）+ 最差穿透 X.X cm @高度（终态全粒子壳穿透一次扫；worstPen > tooSmallPen(0.5) 或裆穿透未解 → tooSmall 偏小信号，UI 注明「读数提示，不改版型」）+ capped（预算尽收束）/frames。**掉裆后裤脚及地 = 真实观感非 bug**（腰锚 7.644 − 前掉 ~7 → 下摆到地被 collideGround 钳 0，真人掉裆裤脚拖地的对应；旁挂 hangLift 12 才有离地余量）。**交互**：滑杆/身高变化不重跑穿台（大效应内经 ref 冻结读取——独立原则），stale 提示「人台已调，穿着待更新」+ 重新试穿按钮（dressEpoch++ 重跑、morph 权重按点击时刻快照）；**独立原则新边界**：穿台碰撞体 = 人台几何（显示层参与穿着解算）不破参数级独立——滑杆不改 payload、读数不改版型/体型。**穿台热力图（同日后补，用户拍板双通道切换）**：侧栏开关 + 通道 Segmented（间隙|应变），`garment/heatmap.ts computeHeat(sim, mode)` 纯函数逐粒子标量——**gap 间隙** = nearestRingBoundary − CORE_SKIN（与 collide 推出位同口径：0 恰在接触壳、负穿透；无环行/超 margin 钳 gapMax(4) 松端；field null 全 0 中性）/ **strain 应变** = 参与的 dist 约束 (len−rest)/rest 的 signed 最大值〔最被拉长的一条边；dist 存 part 局部号、消费方加 offset——与 drape 同口径；NaN 哨兵兜底无约束顶点→0〕；`heatColor` 色带两通道同向**红紧/绿适中/蓝松**〔gap t=clamp(v/gapMax)、strain t=clamp((strainMax−v)/2·strainMax)——+max 拉伸绷紧红、−max 压缩堆布蓝〕；渲染 = rider/sim 视图顶点色 attr（贴层顶点按绑定重心权重插值三宿主粒子、腰头直取本 part）+ vertexColors 材质切换、null 恢复分色；**开/切只重着色当前帧不重跑仿真**（heatOn/heatMode 经 refs 镜像不进大效应 deps，解算中每 HEAT_PRIOR.every(6) 帧刷、终态停走刷末帧定格）。
- **腰圈钉环（2026-09-19 形随体长随衣，穿台现行）**：用户口径「模拟真实的情况、腰头尺寸多少就是多少、穿不进在热力图体现」→ 穿台腰口钉目标 = 钉环 `placement.buildWaistRing(field, y, C)`：腰站 y 行截面边界折线（前中 +Z 最远点起、向 −X 行走，与 band.ringWalk 环序一致）等弧长重采样（`DRESSING_PRIOR.waistRingStep` 0.25）后绕原点整体放大 s = C/周长——**形状来自人体、尺寸来自布长（环长严格 = C），互不锚定**：偏大 s>1 全场体外（间隙随截面比例 0.05~0.25 非均匀）、偏小 s<1 穿体（钉 XZ 冻结如实呈现，热力图 gap 红区 = 穿不进读数）；C 优先腰头带底净长 `bandBottomChain.runLength`（收省后口径）、回退腰站 `girth_finished`。`buildFullPair` 第 8 参 `waistRing` 穿台分支（缺省不传 = 旁挂 θ 角度表原口径字节等价）三处消费：①顶链沿钉环弧长摆放（纸样弧 ×kScale = 钉间距纸样边长零应变；中缝/侧缝腰角由弧长自然落位——扁截面下侧缝腰角前移是布量分布的真实几何，不钉死 0°/±90°）；②侧缝语义竖直线方向 = 钉环侧缝腰角方向、侧腰角共点 snap = 钉环角弧位点；③**带顶两环**：顶环 = buildWaistRing(field, 环源行+带宽, C)（腰上围收窄但前腹外凸——带顶沿用腰站环 XZ 前腹嵌体 0.3~0.6；顶环随行取形 = 上方收窄款真实带顶悬空、凹背行 ~1cm），带列 XZ 随 v 自底环直线渐变顶环位、顶环构建失败退化竖直列。钉集本身不动（（九）腰口区全钉口径）。**热力图 gap 带符号**：`(粒子−最近边界点)·外法线 − CORE_SKIN`（旧无符号 d−skin 把体内深点误读正松；穿体负值红端）。
- **脚口环带刚度（2026-09-19（二）现行，前缘脱扣修复）**：真实牛仔裤脚口 = 双折卷边+车缝硬圈，抗剪切压折——`DRAPE_PRIOR.hemBandStiffness 0.5 / hemBandSpan 4`：弯曲约束中点落在 hem 链纸样 4cm 窗内者经 `drape.stampStiffBands`（三窗之一，优先于缝口窗判；重叠区两者同值 0.5）覆写 `mesh.bendKArr` 至 0.5，环口保持圆形——穿台掉裆加深（诚实掉裆不钳制，dropF 7.12 量级）时脚口前缘仍挂扣脚背至 done（金标 dress.test：脚口带纸样 y∈[−1,1] 前向 max z ≥7.0，挂扣态实测 ~7.9/脱扣态 ~3）。旁挂链同经 buildDrape 生效（四夹具自由垂金标全绿）。k/span 窗口地图与「span 只对本 fixture 对成立（collisionSkin 先例口径）」局限见 priors.ts 注释与决策日志当日条。
- **站点与读数口径**：站高 = targets.json 的 vendor 地标检测值（裆=2环→1环拓扑合并、臀/腰=围度切片极值、膝/踝=右腿围度极小、小腿肚=右腿围度在膝/踝两极小间的局部极大；thigh=裆−3），非启发式。`readGirth`（slice.ts）：body 站取**最大环**（下半身无臂，腰/臀恒单环）、leg 站取**最右大环**（右腿，cx 最大确定性挑出）；BIG=25cm 环过滤为防御性口径。站高取 w=0 值，场会微移站高，极端权重下读数有轻微口径偏差，试验场可接受。
- **资产与许可**：数据源 `vendor/makehuman/`（base.obj + rigs/{default.mhskel 骨架, default_weights.mhw 蒙皮权重} + targets + CC0 全文 + PROVENANCE.md 记 repo/tag/commit/sha256；`.gitattributes -text` 禁 EOL 转换保哈希可校验）；运行时消费 `public/bodymesh/{base.bin,targets.json}`（LICENSE.ASSETS.md 同目录）。许可红线（用户拍板）：仅商用安全许可（CC0/MIT/Apache）、MakeHuman 只借 CC0 数据绝不移植 AGPL 代码、SMPL 出局。`scripts/vendor_makehuman.py`（2026-09-13 定型为**切割+站直姿势链**：站直姿势 = 官方 rigs 数据 clean-room LBS〔大腿绕髋 θ_t、小腿绕移动后膝补 θ_k、脚绕移动后踝反补 −θ_c，臀带由 upperleg01 权重自然携带〕+ 下半身切割 + 官方 14 场重映射与增量随帧旋转 + **身高实测**〔步骤 2.6：切割前站直全身网格对 max/minheight 场 w=±1 实测 max y 响应 → targets.json `meta.height = {baseCm 167.36, plusCmAtW1 +72.02, minusCmAtW1 −36.69}`，exit 14 守卫 ΔH 方向/量级；官方 macro 域极宽——±1 实测跨 130.7~239.4cm，预设全落小权重区〕，`--pose apose` 可退回 A-pose 复现口径；腿去外张/内收/四解剖雕塑/派生场/预标定矩阵全部退役——自研姿势手术撕裂臀线教训，「零姿势修改」口径同日退役；**切缝交点/封盖质心合成顶点按边端点原始 .target 增量线性插值/环均值补场增量**——否则场只动 originals 不动顶环缝，顶缘撕出厘米级拉伸带；14 场全受益，身高± 覆盖全部 4311 顶点；Python 金标 `tests/test_vendor_bodymesh.py` 把守——含水密/地标带/官方场方向响应/站直守卫〔腿轴竖直/膝铅垂/脚平底/左右镜像〕、官方场增量镜像对称〔防随帧旋转线性角未归一的单侧翻转——atan2+π 原值对位置是整周等价、乘部分权重后即翻转，vendor exit 13 域守卫同防〕与身高契约〔meta.height 域 + 预设 155~170 可达性换算回环；身高场方向响应 + 脚底锚地 min y ≥ −1〕）。
- **测试**：`bodymesh.test.ts`（parseBaseBin 布局契约含尾部残留/索引越界防爆 + morph w=0 逐位恒等/稀疏叠加/源数组不被改写 + **height.ts 纯函数**：heightCm 正负两支独立/weightFor 互逆 + 出域钳制/stationFactor 恒等与预设因子 + **真产物冒烟**：盘上读现货 base.bin+targets.json，计数对账/六站齐全/14 场名/heightInfo 实测契约（基高域、ΔH 双向、身高场覆盖全网格）/身高场方向与脚底锚地/官方 waist+ 与 calf+ 自家站方向响应；devDep @types/node，`import.meta.dirname` 定位）；**穿台金标族（2026-09-18 十期起）**：`dressfield.test.ts`（placement 穿台三原语合成场单元：shiftPositionsY 只动 Y/不改写原数组；nearestRingBoundary 环内点法线朝外〔±0.05 跨界验证〕/环外近壳精确距〔16 边形顶点恰在 +x 向，(6,0) 最近边界 = 顶点 d=1.0 精确〕/双环腿间点归最近环；buildLegAxisFromRings fork 搜索/线性插值/域外钳端/无分离行与不足 2 行两 throw + yMin 场负 y 域有环/行号与 forkY 按 yMin 换算 + ankleY 止踝〔脚环 r+|cz| 放大口径不进 rProf〕两用例）、`settle.test.ts`（状态机全迹线——字面量 stub sim + opts.probe 注入：hold 不动钉/frontness lerp 一次下放三钉 y 精确/持续穿透钳 maxDrop→fault→confirm→settle→sim.settled→done→report 全字段/report 接触三态与 −∞ 钳 probeMargin/wake 清假 settled 续预算/maxTotalFrames 硬顶/frozen 直出；probeCrotch 合成单环场壳口径 + yLift 行选择；坑：dropB = −hB 得 −0，toBe(0) 严格比较失败须 toBeCloseTo）、`dress.test.ts`（真 base.bin + fixture 整裤全管线集成 = Fitting3DView dress 分支的无渲染镜像：anchorLift canary ∈(7.0, 8.3) + 人台腰地标 vendor 契约/θ 对齐 canary〔front L rise 末顶点 z>0、back L cb 末顶点 z<0〕/初摆位穿透深度 <0.05〔2026-09-19 钉环口径起 pointInRings 计数把门退役——钉环弦重采样内切伪差 ~0.01cm 级须放行〕/腰圈钉环三组断言〔环长 <0.5%、顶链 ringWalk 连续环走弦和 <3%——per-part 分段漏 4 角点缺口 ~8% 是量法坑、钉集带符号 gap min>−0.06·中位 <0.5·max <1.1（判别「非旧 1.2 垫」用中位数）〕+ 偏小款 C−4 只摆位快检〔钉环整圈嵌体 >100 + computeHeat gap <−1.0 红区，「穿不进」把门〕/落位 done 无 NaN 裆四尖两两 <1.0/dropF ≥2.0 且 ∈[0, maxDrop+ε]/minY ≥ −1e-6〔掉裆后裤脚及地判读〕/脚口前缘挂扣〔纸样 y∈[−1,1] 前向 max z ≥7.0——hem 环带刚度修复金标，挂扣态 ~7.9/脱扣态 ~3〕/**同输入双跑 DressReport 逐字段相等 + pos 数组相等**〔确定性红线——单跑 floor 比较无意义〕/hips+ 负松量探索例宽松断言〔偏小是读数不是错误〕；超时第三参——vitest 无配置文件默认 5s，长集成单跑 ~230s、**全量并行负载下 286s**，双跑用例给 480_000/探索例 300_000）、`heatmap.test.ts`（2026-09-18 热力图：合成 sim+环场——gap 带符号〔2026-09-19：环心 −5·cos(π/16)−CORE_SKIN 负红端、边中点法线外近点正 gap 精确〕/超 margin 与空环行钳 gapMax 松端/yLift 行换算逐位同环心/field null 全 0；strain 逐顶点参与约束 signed max、rest=0 守卫、无约束顶点 NaN→0、dist 局部号+offset〔与 drape 同口径——首版用例误传全局号即炸，口径用例钉死〕；heatColor 两通道端点/中点/越界钳，strain 方向钉死 +max 拉伸=红〔首版 (s+max)/2max 把拉伸映到蓝端是反向 bug，已修〕）；`assemble.test.ts` 另增穿台腰圈摆位 describe（2026-09-19：合成非均匀收窄椭圆场〔只收 a 侧向——均匀缩放径向不变，出不了「顶环更拢」〕5 用例：环长 = C <0.5% + 顶链逐点贴环/角点弧位落环〔方向不定式——漂移方向 = 布弧分数 × 截面弧速分布，真人前移 ~73°/对称椭圆 ≈0，合成场 θ 表全 0 时 x>5 即证走钉环而非径向场〕+ 前后宿主角点共点/两环带顶收窄更拢 + 两缘弦和 ≈ C〔带顶 2.5%——端帽列 v<带宽直线割线略膨胀〕/LR 镜像/偏小环 C−4 整圈嵌体）。引擎金标 `python -m pytest tests/test_fitting_payload.py tests/test_vendor_bodymesh.py -q` 照旧。

**（史实存档）退役清单（2026-09-12~13 多批；机制细节、金标口径、验收复验全录决策日志 §十一。**①的 PBD/缝合/热力图与②的 payload 前端消费已于 2026-09-13 二期复活**〔garment/ 新写 + worker/ 重写，非 git revert——碰撞换 three-mesh-bvh、腰头升真实布片〕；②的围度闭环部分仍退役**）**：①试穿链——布料四半片/腰头环带/结构线/应变热力图/PBD 解算/三管高度场碰撞（`pbd/`、`seams.ts`/`mesh.ts`/`structureLines.ts`/`heatmap.ts`/`useFittingSolver.ts`、`bodymesh/heightfield.ts`）与 mannequin 适配层（`mannequin.ts`）；②体型围度调节链——`BodyProfile`/体型库 UI/localStorage `ylpattern.body.v1`、`bodymesh/` 运行时（load/morph/align/calibrate/build）、`useMannequin.ts`、`priors.ts`、fitting payload 前端消费（`postFitting`、protocol `'fitting'` cmd、`types.ts FittingResult` 契约、useDraft `generateFitting`）与「生成」按钮/StaleFlag。③静态 raw.obj 直显（rawMesh.ts）同日让位滑杆试验场。④base.bin 姿势链前端滑杆路径（bodymesh/load.ts）半日后让位全身原生链。⑤vendor 派生链 2026-09-13 重写为纯切割链（脚本由 ~1700 行瘦至 ~560 行，派生场/雕塑/标定矩阵随历史归档决策日志）。⑥全身原生链（native.ts + raw.obj 运行时消费 + targets/measure 拷贝）2026-09-13 同日让位——用户口径落定为「只显示下半身」：前端切回 base.bin（纯切割版），raw.obj 从 public 删除（vendor/makehuman/base.obj 仍是 git 内原件，恢复全身 = 从 vendor 拷回 + 重建 native 链）。⑦「零姿势修改」（A-pose 原样切割）2026-09-13 晚退役——官方 rigs 三件套探针验证通过后站直 LBS 采纳（决策日志 §十一 当日条目），A-pose 复现 = `--pose apose`。更早史实：旧环模型（SDF smin 蒙皮 + 环放样七轮打磨）2026-09-11 随真人网格换轨整体退役。⑧2026-09-13 复活的整裤缝合解算链（garment/ 试穿装配层 + worker/ sim 解算层）2026-09-15 整体删除——yoke 16 缝整裤不收敛、整体渲染效果不佳，用户拍板推倒逐步重建（一期 = 前片 L+R 静态旁挂；mesh/core/placement/render/priors 地基保留，攻坚史与调参红线录决策日志 §十一 2026-09-14~15 条）。
