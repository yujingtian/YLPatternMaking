...
- **衣片链（六期现行 = 前后身缝合自由垂 + 其余平铺；一期扇区摆位 → 二期引力下垂 → 三期平铺验证+平面拼合 → 四期前身缝合立起 → 六期后身同款 → 2026-09-17 悬挂抬升+顶部圆弧的演进见决策日志 §十一）**：useDraft.generateFitting（protocol `'fitting'` cmd 引擎优先回落 HTTP，payload 照旧全片）→ 前端消费全部片，两路分流——**前/后身组立体链**（`panel.buildFrontPanel` / `panel.buildBackPanel` 并集宿主 → 悬挂链，见下段）+ **其余裁片平铺** `assemble.buildFlatLayout(payload, exclude)`（exclude = 前身组锚 front_piece〔panel.hasFacing 再加 front_facing〕+ 后身组锚 back_piece〔backPanel.hasYoke 再加 back_yoke〕：排除锚且贴合守卫通过 → **整组**离开平铺〔缝合整体要么全平铺要么全立起〕，守卫不过组员照旧独立行 = panel 退化兜底；内部逐片 `garment/mesh.buildClothMesh`〔边界弧长重采样+错排栅格+delaunator 质心过滤；内部三角形最小角 >15°、面积比 ≥99%，边界细长三角形原样；距离/弯曲约束与边界环字段为解算重建预留〕后**行式平铺** y=0 地面——payload 逐片一行〔行序 = 片序、行距 FLAT_PRIOR.rowGap(6)〕；**缝合拼合组 STITCH_GROUPS**（结构化：members + **贴合守卫 aligned**——缝合依据线双向最近点距 max <STITCH_GAP(0.5cm)〔seamGap，edgePts 按边名取链 / facing 用 marks〕才拼合，不贴合或组员缺片退化独立行，**不错位硬拼**）〔晚八 back+yoke「先将机头和后片缝合成为一个整体」→ 晚九 front+facing「将前口袋也平铺出来，然后和前片缝合，形成一个整体」〕：组内成员按**整版全局坐标**对齐（共享组 bbox 归一变换）= 缝后位置天然贴合——back_piece+back_yoke：back 上边 top = yoke 下边 bottom（整版同一净线，无省款 fixture 实测逐点重合 0；**有省款 yoke 下边是省闭口净样、与整版下口线错位实测 ~12cm = 后片省未收的真实缝前状态，守卫拦下退独立行**，处理留后）；front_piece+front_facing：缝合依据 = **facing 袋口净线（marks，任一条）↔ front mouth 边**——直/弯腰头两款 fixture 实测 marks[0] 与 mouth 逐点重合 0（facing 外边 1:1 复制挖削**前**大片轮廓、按整版坐标摆位自动补齐袋口月牙缺口，袋口净线即缝合线）；**组内叠层**：第 j 片 y = j×FLAT_PRIOR.stackStep(0.2)——袋贴袋口条带与前片净样重叠，共面 z-fight 微抬分层；**方向口径（2026-09-16 用户拍板「前片在前口袋上面」）**：members = [front_facing, front_piece]，facing 片序在前 = 下层、前片叠上层——俯视袋口条带区显示前片蓝色（外观主体是前片，袋贴衬里侧）〕；成对裁片〔front/back+yoke 拼合组/facing 及后续加片〕L 原样归一 (u, 0, 行基 z+v) + R **整组**镜像 u→W−u 平移 L 右侧留 FLAT_PRIOR.gap(4)〔拼合组关于组中线镜像——真裁剪 yoke 也是 L/R 两片〕、腰头单片〔SINGLE_PIECES 整根幅样无左右之分、side 固定 'L'〕，全组 z 居中〔组中心 z=0〕；bbox 由网格顶点实算不信任 payload 标量；无弯折无解算、网格即净样原形，与 2D 裁片 SVG 一比一；**验证分流**：平铺即错 = payload 边链/网格层问题〔引擎 build_fitting_payload 或 mesh.ts 上游〕，平铺对悬挂错 = 摆位/解算层问题）→ `render.buildGarmentView`（**逐片分色** PIECE_COLORS：front 蓝/back 绿/waistband 橙/back_yoke 紫/front_facing 青、未登记片灰兜底；每片一 Mesh 一材质，`pieceColor(key)` 供 UI 图例同源取色；Fitting3DView 侧栏「裁片」卡图例 = **固定片序**〔PIECE_LEGEND_ORDER：前/后/腰头/育克/袋贴〕色点 + 中文名，payload 没有的片灰显「未开启」+ hint 标开启路径——晚九后排障：用户报「前口袋没渲染」实为 front_pocket/front_pocket_facing 默认关、payload 无第 5 片非渲染 bug，图例固定序让缺片一眼可见〔2026-09-16 起 Web 初始参数**产品层默认开**口袋族（useDraft 初始化对 localStorage 缺键补 front_pocket/front_pocket_facing = true——「标准牛仔裤」口径，引擎 PatternOptions 默认保持最小裸版：extract 探针守卫拒绝口袋特征 + 部分 extract 测量组合 facing 默认参数几何越界，改引擎默认当日回滚，详见决策日志 2026-09-16 条）；灰显 = 用户显式关过对应开关〕）；显示层平铺 Group `position.x = backX + envelopeR + FLAT_PRIOR.clearance(6)`（让位到后身悬挂筒右侧；hangX/backX/envelopeR 见下段立体链；morph 增量由净空兜底）、`position.y = FLAT_PRIOR.lift(2)`；hangXRef 存**全场景包络中心**（人台左沿 ~ 平铺组右沿的中点——双主体 = 人台+悬挂筒 ↔ 平铺组），相机 target/机位按场景中心差量平移居中〔幂等〕，flyTo 四预设同口径。**前/后身缝合立体链（六期现行接线）——panel 并集宿主 + rider 贴层 + drape 自由垂**：`garment/panel.buildFrontPanel(payload)`（**前身并集宿主**：front_piece 边链 mouth 段〔free〕原位替换为袋贴月牙边——mouth 两端点投影到 facing 的 waist/side 边取部分段，waist 部分段命名 `'waist'`/top_chain、side 部分段命名 `'side'`/seam，与 front 同名相邻边 **mesh runs 自动聚合** → drape 悬挂 pin 语义自动覆盖袋贴腰口段〔整个前身挂腰，物理正确〕；**几何依据**：facing 外边 1:1 复制挖削**前**大片轮廓，袋口月牙区在 front 净样**之外**——facing 不能直接绑 front 网格〔月牙区顶点 locate 全 null〕，必须并集；守卫 = `assemble.frontFacingAligned`（与平铺拼合同口径提炼共用：marks 与 mouth seamGap <STITCH_GAP）+ mouth 端点投影 <JOIN_TOL(0.5cm)，失败退化纯前片宿主 + 袋贴留平铺〔warning 上屏提示〕）与 `garment/panel.buildBackPanel(payload)`（**后身并集宿主（六期）**：back 边链 top 段〔机头下口线 P0→PN，有 yoke 时角色 seam〕**丢弃内部化为缝线**，数组末尾接 yoke 余边反向闭环路径〔cb P0→O + top O→X + side X→PN，reversedEdge 端点对调〕——**重排聚合关键**：back.cb〔裆尖→P0〕与 yoke cb 反向段〔P0→O〕同名相邻，runs 聚合成**整条后浪**（裆尖→腰口贯通育克，yoke fixture 29.0cm）→ drape 后中缝合对沿 cb 配对天然缝到腰；yoke 腰口 top 反向段顶替 top_chain〔悬挂 pin 自动覆盖〕、yoke side 反向段与 back side 同名接续；守卫 = `assemble.backYokeAligned`（与平铺拼合同口径提炼共用：back.top ↔ yoke.bottom seamGap <STITCH_GAP，无省款 fixture 逐点重合 0）失败退化纯后片宿主 + 育克留平铺；**无育克 = 正常款型**纯后片即后身、信息性 warning 非守卫失败）→ `garment/rider.bindRider/rideRider`（**贴层**：前片/袋贴不进粒子系，每顶点 host.locate 重心绑三角形〔重心权重对仿射变换精确，verlet 帧间分段线性零额外误差〕，L/R 共享一份 2D 绑定〔hostOffset 0 / hostN 区分两侧〕；**locate 缺口兜底 = 宿主边界环最近段插值**〔Delaunay 无约束+质心过滤在凹弧丢窄楔形三角形——月牙外边恰是凹弧，实测 facing 11/76 顶点命中；段插值 = 弦近似弧、误差亚毫米，远优于最近角〕；`rideRider(b, hostPos, hostOffset, out, radialOffset)` 重心重建后按 `(r+radialOffset)/r` 径向缩放 px/pz〔y 不动〕——袋贴 −HANG_PRIOR.riderStep(0.2) 衬里侧）→ `render.buildRiderView`（每 side 一 Mesh、逐帧回填 + computeVertexNormals，材质口径同 buildGarmentView、PIECE_COLORS 逐片分色）→ `garment/core.buildCore`（**纸样围度撑型芯**：girth_finished 逐站取值、芯半径 = g/2π − CORE_SKIN(0.98)，单一水密花生网格——腰臀圆环 + 裆下双瓣花生整环〔瓣周长锁定 2×腿站 girth、腰谷 ε 平滑、裆带 ±3cm 圆↔花生混合；**裆站躯干圆半径 = coreRadius(max(臀, 2×腿))** 毗围代理防掐腰——直取单腿半径会把臀→裆掐成蜂腰再弹回双瓣〕，v1 圆筒+双管+龙骨四条死路见 core.ts 头注；摆位壳 core+skin+gap 落纸样围度 = 「圆筒按构造成立、廓形=版型」）→ `placement.buildBodyField`（支撑半径场 R(y,θ) 凸上界查表）→ `assemble.buildHangPair(key, host, field)`（前身/后身悬挂对共用，2026-09-16 六期由 buildFrontPair 泛化：宿主 × {L,R} 扇区摆位——front 沿前 90° 扇区 L θ∈[−90°,0°] 侧缝→前中、back 沿后 90° 扇区 L θ∈[−180°,−90°] 侧缝→后中（R 均镜像 θ→−θ；侧缝两扇区同归 ±90°、中缝相对 0°/180°——穿着拓扑同构），半径 = 场 + garmentGap 1.2；x→θ 均匀映射沿用旧初摆位口径的已知近似，弧长随站半径微变；**腰口弧长重参数化（六期「拉直」）**：x 映射固有偏心——腰口两端点不落扇区边界（中缝腰角实测偏心 front −3.9 / back −5.8cm），二轮 Y-only 下靠环滑自愈、三轮整圈钉下会留顶中缺口/鞍——顶链顶点在 x 映射后改按**腰口弧长分数** s∈[0,1]（s=0 = 中缝腰角端，中缝链腰口端顶点定向；无中缝款回退链首）重铺：中缝腰角精确落 0°/−180°、侧缝腰角精确落 ±90°、θ 沿链单调，y/半径口径不变、R 侧 θ→−θ 镜像求场；片身其余顶点照旧 x 映射（初摆位近似，解算自松弛）——这是 drape 整圈全向钉「钉与缝同意」的摆位侧前提；**侧缝边语义摆位（2026-09-17「顶部侧缝边不要折、拼合后顶部〔腰头缝合线〕是圆弧」）**：x 均匀映射对弯曲侧缝（牛仔裤腰口撇势内收）把腰口段侧缝摆到 θ≈−76°、与重参数化侧缝腰角（精确 ±90°）在腰口角正下方留 ~14° 楔形折角——侧缝边在穿着语义上是体侧竖直线（纸样里的弯曲由绕体 wrapping 吸收，与腰口重参数化同因），'side' 名 run 全量整条改摆 θ=±90° 竖直（r = 场+gap、y = 纸样高原样；后宿主育克侧段与后片侧缝可能各自成 run，全覆盖）；**悬挂整体抬升 hangLift(12cm)**：布全长 > 腰口挂高（筒面斜挂+腰弧下沉+拉伸，实测余量 ~8-10cm），不抬下摆触地堆布（front 102 / back 202 粒子贴地实测；抬后三款夹具零触地、离地 3.3~4.4cm）——全 pos y += hangLift 在所有摆位之后统一施加（含钉目标），场查询仍用纸样 y；L/R 共享**同一宿主 ClothMesh 实例** → rise/cb 链同号配对天然可用）→ `render.buildGarmentView`（每片一 Mesh 共享白坯材质 DoubleSide）→ `drape.buildDrape/stepDrape`（二期新增，用户口径「裤子的下垂要符合地球引力」：主线程轻量 verlet——**pin = 腰口整圈全向钉（六期「拉直」三轮现行，无例外）**：真实「提着整个腰口」——顶缘按摆位弧撑住、布条自顶缘竖直垂下，中缝不再塌向轴心〔悬挂支点；**角点口径**（边界重采样每边含起点采样、不含终点，共享角点 = 下一条边首采样）：终点角必须入钉集否则急坠（实测侧角 −2.1cm）。演进三轮：一轮（2026-09-15 顶部折角）全向钉整圈把腰口锁成刚性折线——顶缘呈「图案角+侧缘陡落」尖状凸起、前中角锁在均匀映射偏心位（实测 −3.9cm ≈ 17°）与前中缝拔河出顶中缺口；二轮 Y-only+双角锚（单锚实测 600 帧不收敛）——布自重圆化顶缘、前中缝合顶缘滑到正中闭合，但环向可滑 = 布自重把整圈腰口滑向轴心：中缝荡离前中/后中（前中缝外伸 16.6→5.7cm）、整幅布塌成对折门帘（2026-09-16 用户报障「前中和后中往里折、要拉直」）；三轮（现行）= 整圈全向钉——一轮拔河的根源是**映射偏心而非钉本身**，摆位侧腰口弧长重参数化（见下 buildHangPair）把中缝腰角精确摆到中面后「钉与缝同意」按构造成立（实测缝对 avg/p95 = 0），顶缘全撑 = 直挂〕、**侧缝边顶部刚度带（2026-09-17）**：side run 顶部 sideHold(10cm) 并入全向钉集 = 腰头缝合线的刚度带（真实成衣此区由腰头撑圆；用户拍板两筒并排不缝侧缝、顶部撑圆）——无侧缝缝合的自由半身筒侧缝自由边自由垂中向筒轴心内摆（后身实测最深 ~25°）、顶部扇区塌角；配摆位侧语义摆位后顶部圆弧保持、带缘出口偏差 ≤0.7°（往下自由内摆渐增属自然垂）、约束 = mesh 预留 dist/bend 数组 + **中缝对**〔L/R rise|cb 链同号镜像配对 rest=0，2026-09-15 同日用户拍板「前片先将前中缝合在一起」、2026-09-16 六期后身同款后浪 cb 链接入；两片共享同一网格、顶点号天然一一对齐〕、**碰撞 = 自由垂（五期起现行，六期直挂沿用——指无撑型芯碰撞）**：`buildDrape(garment, field)` 的 field 可空、Fitting3DView 传 null——径向芯碰撞短路不用（芯撑出的前凸筒不是真实提着前片的形态），主约束 = **地面碰撞** collideGround〔y<0 推回 + 法向速度清零 + 切向摩擦 0.8 防滑转；悬挂整体抬 hangLift 后正常挂相零地面接触（实测离地 3.3~4.4cm），碰撞降级安全网〕；芯/场只供初摆位半径与取景包络；重力 −980 cm/s²；Fitting3DView rAF 实时步进至 settle/封顶，直挂实测 front 184 / back 186 帧真收敛（抬升+刚度带口径）、缝对 avg/p95 = 0——中缝保持在前中/后中（不塌轴金标 ≥55% 摆位外伸实测通过），下摆离地 minY 4.4 / 4.0；无跨片装配缝合/无 worker/无 BVH；常数 DRAPE_PRIOR 继承已删 SOLVER_PRIOR 标定值〔git d784de1〕）。**形态残留沿革**：一期静态直出时裆下前中有明显折槽（裆点恰落 θ=0 且芯花生腰谷沿前中径向断崖 y80→y74 掉 5.5cm、逐行谷-侧差 6~9cm；均匀角度映射把裆下布平摊贴谷壁——**布去向错位**而非布量过剩，y68 行内余量仅 0.3cm），二期引力下垂已消解（裆下布沉入腰谷自然悬垂）。**前中闭合**：未缝合自由悬挂时前中曾敞开 ~5cm 缝（内边 rise 链荡至 θ≈−16°~−11°，物理真实），现由缝合对闭合；**fly 连裁款 rise 链缺失则跳过缝合**（前中敞口属连裁门襟固有形态，缝合范围只到裆点——inseam 是前后片缝，留给后续加后片）。（裆部毗围代理环比臀突出 ~5cm 只影响芯网格本体——五期自由垂后芯不进碰撞，该前凸观感随芯碰撞退役。）**显示层偏移（旁挂不套轴）**：前身 rider 视图 Group `position.x = 人台基础网格半宽 + HANG_PRIOR.clearance(6) + envelopeR`、后身 Group 并排 `hangX + 2×envelopeR + clearance`（envelopeR = field.maxRadius + garmentGap；独立原则——CORE_SKIN 只把芯半径锚**纸样围度**〔g/2π−0.98〕，与人台围度无约束关系，套人台轴必穿体）、`position.y = HANG_PRIOR.hemLift(2)`（历史防裤脚与地面格网共面打架的「白色交叉线」观感；hangLift 抬升后解算本体已离地 3~4cm，本值仅余显示量）；**初始摆位直接可见，随即 rAF 引力下垂动画至静止**（无缝合/松量读数 UI）；相机 target/机位按 offset/2 差量平移居中双主体〔幂等〕，flyTo 四预设同偏移。**独立原则（用户拍板）不变**：人台滑杆不改衣服、打版参数不改人台；交互 = 重新生成（重发 payload）+ 自动首次生成 + fittingStale 提示；**绝无自动调版/自动改体型闭环**。金标 `panel.test.ts`（前身：mouth 内部化〔替换后无 mouth 边〕/waist 聚合变长〔袋贴腰口段并入 pin 域〕/**覆盖性 = locate 非空或贴宿主边界 <0.1cm**〔并集完备性的正确验收——边界三角形覆盖天然有缺口，locate 非 null 过强〕/面积比 ≥99%/两路退化〔无 mouth、守卫破坏——**测试构造平移 facing 必须 marks 连同 edges 一起平移**，marks 是独立字段只动 edges 守卫照过〕）/`rider.test.ts`（平面恒等分层容差：非兜底 1e-4 / 兜底 = 段插值弓高 <0.1；刚体跟随仿射封闭；径向内偏〔大半径柱面构造验证「半径 −0.2、y 不变」〕；兜底占比 front <5% / facing <25%〔凹弧边界点实测 11/76〕）/`garment.test.ts`（mesh 拓扑/场与摆位/穿透口径）/`core.test.ts`（水密单环/芯周长逐站锁定围度/裆站不掐腰）/`assemble.test.ts`（三期全裁片平铺金标：parts 展开 序〔成对片 L+R、腰头单片〕/全片贴地 y≡0 + 全组 z 居中/行序 = payload 片序 + 行距恰 rowGap/成对片逐点等距变换 L 原样 R 前中镜像/腰头单片原样/+ 缝合拼合金标〔fixture_fitting_yoke.json 从 git d784de1 恢复（schema v1 未变直接兼容）：拼合行 parts 序 yoke 不独立成行/back top 边与 yoke bottom 边逐点贴合 <1e-4cm（L/R 两侧）/整组镜像/拼合行总高 = y 并集〕+ 袋贴拼合/守卫金标〔fixture_fitting_pocket.json **新生成**（引擎 FlowRunner(front_pocket+front_pocket_facing) 直出直腰头口袋款，补 ok/warnings）+ fixture_fitting_curved_pocket.json 从 git d784de1 恢复（弯腰头+有省 yoke）：拼合 parts 序/缝合依据数据前提（marks↔mouth 双向 <0.5）/**组共享变换断言**（L 侧 pos = xy − 常数 Cx/Cy 逐点恒定、R 侧翻号——「整版对齐」的本质断言，比摆位后测边距更强）/组内叠层 y（front 0、facing = stackStep）；守卫：有省 yoke 错位 ~12cm 拦下退独立行、弯腰头袋贴照常拼合〕——**坑两个：`Garment.pos` 是 Float32 渲染 buffer 口径，对照 Float64 `xy` 有 ~5e-7cm 量化伪差，toBeCloseTo 精度放小数 5 位（9 位必炸）；buildFlatLayout 内部自建 mesh，测试侧再调 buildClothMesh 是引用不同的新对象，toBe 引用断言必炸——内容一致性由逐点等距变换用例钉死，结构对账用顶点数**；悬挂金标保留：L+R 镜像对称 z 差 <0.1cm〔撑型芯场 θ-bin 量化伪差，人台场旧标 0.05〕/前扇区角度域/穿透按构造为 0〔查验回纸样空间——hangLift 抬升后 pos y ≠ 场查询 y〕+ 侧缝语义摆位金标〔side run 全顶点 θ=±90° 竖直、y = 纸样高 + hangLift；全 pos y ≥ hangLift〕）/`drape.test.ts`（前身/后身三 describe 共用 assertStraightHang（seamDir = 中缝朝向 +1 前中/−1 后中；六期「拉直」随整圈钉换代由 assertFreeHang 更名）：600 帧内真收敛非封顶/整圈钉恒守初始位 + 钉数 ≥ 2×(顶链顶点+终点角)/中缝 avg<0.3、P95<0.8〔旧整裤缝合金标口径；重参数化镜像摆位起点即闭，实测 avg/p95 = 0〕/**不拖地 minY>1**〔2026-09-17 hangLift 抬升后零地面接触，实测 3.8~4.4；地面碰撞降级安全网〕+ **侧缝顶部刚度带金标**〔side run 顶部 sideHold 带终态 θ=±90°（L −X / R +X、z≈0）= 顶部圆弧不折的量化〕（质心沉降断言退役，实测 Δ≈0）+ **六期拉直形态金标（用户口径「前中和后中往里折、要拉直」的量化，替五期裆尖内收/缝线不凸两条——直挂下中缝本就该留在外侧，收缩断言失效）**：①中缝腰角居中 |x|<0.1cm（重参数化 + 镜像钉位，「钉与缝同意」直接验收，实测 0.000）；②中缝不塌轴——终态中缝链缝向**平均**外伸 ≥ 摆位 55%（Y-only 口径前中缝均值曾塌到 ~5.7 对折门帘；实测现 front 87% / back 65%）；③缝不外凸——终态缝向最大外伸 ≤ 摆位 +0.5cm（实测 front 16.61→15.85 / back 16.08→11.09）+ **并集宿主收敛金标**〔pocket/yoke fixture 全链：月牙带/育克拼入不破坏既有口径，四/六期最大数值风险点——yoke fixture 后身 249 帧真收敛、后中缝 P95 达标、后浪贯通 cb 聚合链 29.0cm〕；assemble.test.ts 另增 exclude 四用例〔排除锚整组离开 / 只排锚组员随走 / 无袋贴款无组员 / 守卫破坏组员照旧独立行——**期望行序记 payload 片序，front_facing 在 back 之后**〕+ 六期后身组三用例〔exclude back_piece+yoke 整组离开 / 只排锚育克随走 / 守卫破坏育克照旧独立行〕+ 后片 L+R 后扇区摆位金标〔θ∈[−180°,−90°] 角度域 / 镜像对称 z 差 <0.1cm / 穿透零〕+ 腰口弧长重参数化金标〔前/后身并集双宿主：中缝腰角 |x|<0.01 落中面 / 侧缝腰角落 ±90° 且 L 侧 −X / θ 沿链单调铺满〕；garment.test.ts placePoint 增 back 后扇区金标〔侧缝 −90° 与前扇区同角、后中 −180°、R 镜像两半后中同在 −Z 闭合〕），vitest node 直跑。**整裤缝合解算链（2026-09-13 复活版）已于 2026-09-15 整体删除**：seams/band/ease/heatmap/align/useGarment、worker/{entry,protocol,solver,collide}、pbd/constraints、yoke 与口袋变体 fixture 及其金标（solver/hang/ease 测试）、three-mesh-bvh 依赖；yoke 16 缝不收敛（全局摆振泵）的攻坚史、调参红线（collisionSkin 0.98 持续接触锚、丹宁增硬 0.45~0.5 助收敛、preRelax 300、crotchHold 60 硬释放、band rest 全保纸样矩形等）与已证伪方案（无碰撞单帧塌片 wrapHem 0.50、全钉腰头卡死、摩擦/阻尼/迭代单 knob 全反证）录决策日志 §十一 2026-09-14~15 条——**重建解算时先读**。
