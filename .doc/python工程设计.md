...
- **体型与成衣分离（用户口径 2026-09-09；人台固定 2026-09-11）**：尺寸单围度是成衣量（含松量/调节量）不能当人体量。`BodyProfile`（腰/臀/大腿/膝围）是**纯前端态**（localStorage `ylpattern.body.v1`），不进引擎 Measurements/DraftPayload、不 bump 参数版本号、不影响打版。估计体型 = **首次出现有效尺寸（waist>0 防零快照）时按 `estimateBody = 成衣 − 先验松量`（clamp ≥0.85×成衣）快照进 `store.estimated` 持久化**，此后成衣参数变化零联动（人台固定口径：设置好多少就是多少，不随裤子变），显式刷新走抽屉「按成衣尺寸重估」（重估+激活一步）；未快照前实时估计仅作兜底。体型管理三件套：内置预设 5 个 + 自定义保存管理 + JSON 导入导出（bodyProfileStore.ts）。
- **模块图**：`priors.ts`（唯一先验收敛点：松量/体型比例/PBD/网格/网格人台，全为非引擎数据）→ `bodyProfile(t)` → `bodymesh/`（真人网格人台唯一路径：load/morph/align/calibrate/heightfield/build，见下文「真人网格人台」节）→ `mannequin.ts`（适配层：`Mannequin/Tube/SectionParams` 类型契约 + `sectionAt/radiusAt` 查询，hf 柄短路查表）→ `mesh.ts`（净边链弧长重采样成边界环 + 内部错排栅格 + delaunator 三角化，距离/弯曲约束与边名聚合段 runs）→ `seams.ts`（包裹摆位 + 缝合配对 + 腰口 pin + 环带）→ `pbd/{collide,constraints,solver}.ts` → `structureLines.ts`（marks 重心绑定随布料变形）/`heatmap.ts`（约束应变→红绿蓝）→ `useFittingSolver.ts`（生命周期：rAF 步进、settled 停、tab 隐藏暂停、热启动）→ `Fitting3DView.tsx`/`BodyProfileDrawer.tsx`（2026-09-11 起常驻右栏主视图、不再挂 tabs，进入系统即自动出人台+裤子；人台形体验收期 2026-09-10 曾以 Fitting3DView 顶部 `RENDER_GARMENT` 单开关临时屏蔽布料渲染（布料四半片/腰头环带/结构线与热力图/透明度/重新试穿等布料相关渲染和控件全部跳过、solver 照常解算，man/buildVersion 仍驱动人台随体型重建），人台视觉验收通过当日已改回 true 恢复全量渲染；现行默认 true、裸台核验走 URL 参数 `?garment=0`（Playwright 截图/VLM 验收用，免改源重编；注意裸台仍需合法 draft 参数——左栏 measurements 为空时 fitting POST 422、人台不渲染）。渲染分支已随 SDF 蒙皮退役删除（见下文史实存档），人台渲染 = bodymesh `sourceMesh` 直出。
...
- **PBD**：verlet（dt=1/60、g=−980cm/s²、阻尼 0.985）+ Gauss-Seidel，3 substeps × 6 iterations；约束 = 三角形全边（结构+剪切 1.0）+ 相邻三角形对点（弯曲 0.3）+ 缝合（1.0）+ pin（w=0.9）；碰撞 = 解析投影（y 夹取→sectionAt 插值环→radiusAt（hf 高度场查表）→推到 R+skin 0.3 + 切向摩擦 0.5），骨盆/两腿取最深。热启动：新网格顶点在旧网格重心插值继承位置与 0.5×速度（边名签名变化才冷启动预松弛 200 迭代）。发散兜底：NaN/飞点 → 恢复上一好帧（阻尼×0.9、迭代×2）→ 连败 3 次冻结 + 「仿真已暂停」角标。settled（均速<0.8cm/s×30 帧）停解算只渲染。
- **辅助**：应变热力图（红紧/绿贴/蓝松，约束数据顺手算零成本）、视角快捷（正/侧/背/斜 45°，OrbitControls 自由拖不锁）、透明度滑杆（0~1 统一驱动布料四半片/结构线/腰头环带三材质，**默认 0 进 3D 先看人台**〔用户口径 2026-09-11〕，0=完全隐藏只看人台，热力图同受其驱动）+ 隐藏人台、截图 PNG（同步 render 后 toDataURL）。改参数走左栏（2026-09-11 删 3D 内快捷滑杆卡：与左栏重复；stale = 舞台 Tag + 生成按钮角标，手动生成刷新试穿）；fitting 快照独立、不走「先画后裁」门控——试穿是探索视图。three/OrbitControls 动态 import 独立 chunk，首屏主包零影响。
- **测试**：`fitting3d.test.ts`（mannequin 适配层 radiusAt/sectionAt/estimateBody 金标/体型库/欧拉公式 V−E+F=1/摆位约定/碰撞投影/全 pin settled/应变单调）+ `bodymesh.test.ts`（合成网格金标：slice 切片/morph 稀疏叠加/align 分段 y-warp（含踝节点路径）/calibrate 差分雅可比反演+耦合联立（waist+ 溢出 hips 站一并满足、hips 权重保持 0——旧对角反演在 hips+ 行 null 时永不收敛）+分档钳界（合成资产含 waist+/thigh+ 双 target）/heightfield 三管一致 + fillThetaPits 单测（单坑/相邻双坑/缝上坑抬邻 max；浅谷/光滑宽谷/合法锐台阶/空 bin 不动）/buildMeshMannequin 出口契约 + thigh 钳站测（BODY 带 thigh y=crotch 复刻引擎 thigh_measure_offset=0 口径，站钳回 crotch−3 后闭环收敛、站点围度=目标）——node 无 fetch/public，**真数据金标在 Python 侧** test_vendor_bodymesh.py）+ `fitting3d.integration.test.ts`（真资产端到端：node:fs 读 public/bodymesh 走与浏览器同一 parseBin→buildMeshMannequin 链 → 网格拓扑→缝合配对→150 帧 PBD 有限/缝合闭合<1cm/不穿透（+skin−0.15 强断言）→ 包络坑守卫（裆−6..+12 带 θ=±90° 相邻 0.5cm 行 |Δenv|<2cm；θ-bin 别名坑指纹 8.9））。fixture 重生成命令见 integration 文件头；fixture 用静态 `import fx from './_fixture_fitting.json'`（tsconfig 开 `resolveJsonModule`）——**产品代码勿用 `node:fs`**：tsc -b 浏览器 lib 无其类型，`npm run build` 的 tsc 阶段报 TS2307（2026-09-09 实测；测试文件内可以 `// @ts-expect-error` 紧贴 import 行压制）。

**（史实存档）旧环模型已整体退役（2026-09-11 随真人网格换轨，演进七轮史与机制细节见决策日志 §十一及退役前 git 历史）**：`skin.ts`/`skin.test.ts`（SDF smin 蒙皮 + 查表 marching cubes）删除；`mannequin.ts` 环放样整链（骨盆/双腿超椭圆截面、emergence 外缘/gap 梯子、矢状 S 塑形、脚盒/脐原语，`BODY_RATIO/SECTION_PRIOR/SAGITTAL/FOOT_PRIOR/NAVEL_PRIOR/SKIN_PRIOR` 及 `USE_BODYMESH` 并存开关）删除，`priors.ts` 相应收敛。继承至今的口径：`radiusAt` 只收**单位方向向量**（返回裸 R，skin 由 collideOne 自加）；`collideSweeps=3` 顺序投影非幂等多遍扫描守卫；集成测试 −0.15 容差语义（接触带力平衡残差 / 高度场并集保守带同收纳）。


**一期显式非目标**：自碰撞、小裁片独立缝合（口袋/门襟/贴袋等只画结构线随布面变形）、腰头布片仿真（近无弹性、只引入高刚度小步长稳定性代价）。二期候选见决策日志。

**真人网格人台（2026-09-11 定案换轨：MakeHuman CC0 数据 + 自研 TS morph，旧环模型整体退役见上段史实存档）**：七轮程序化打磨观感到结构性天花板（围度站点放样只能逼近剪影，到不了真人网格的自然细节），换真人网格。许可红线（用户拍板）：仅商用安全许可（CC0/MIT/Apache）、MakeHuman 只借 CC0 数据绝不移植 AGPL 代码、SMPL 出局。
- **数据链（git 内自足）**：`vendor/makehuman/`（base.obj + macro 2 + measure ±8 targets + CC0 全文 + PROVENANCE.md 记 repo/tag/commit/sha256；`.gitattributes -text` 禁 EOL 转换保哈希可校验）→ `scripts/vendor_makehuman.py` → `webapp/frontend/public/bodymesh/{base.bin, targets.json}`；Python 金标 `tests/test_vendor_bodymesh.py` 15 例（数据缺席整文件 skip）。vendor 数据级坑（先探针 dump 定案再动手）：① base.obj 为 **A-pose 绑定姿势**——`desplay_legs` 每顶点 XY 平面旋转（pivot=中轴 (0, 裆+6)，权重 = y 向 smoothstep（裆±8 blend 带）× |x| 向（≤2 会阴中带刚性→≥7 全量，防裆顶点被左右反向旋转撕裂）），**targets 增量随各自顶点同旋转**（morph 语义 = 旋转后场上叠加），自检+金标腿轴守卫 <2.5cm 散布（叉部渐出后轴自然内斜，见④；质心轴本身带解剖弯度，共线口径不可用）；② 旋转使腿微缩脚下探，re-zero 抬升 +3.1cm——height/粗裁/精裁自检基准随之重算；③ 臀地标检测带上移 [裆+6, +14] 避开裆叉喇叭带（双腿合并叉环伪围度 ~115 > 真臀 ~98）；④ **腿内收 `adduct_thighs`**（desplay 只对直腿轴、pivot 在裆上，裆下腿轴间距 ~19cm 原样保留：膝上 gap 5.3~6.5、膝下 6.2~7.7）——逐 1cm 量右腿环内侧 gap，单侧共向平移 δ=max(0, gap−target)/2 × 踝上 6cm 渐入 ramp × **叉部 7cm smoothstep C¹ 渐出**（δ 自裆−2 起平滑升至全量：裆下即满量会把大腿干从臀线正下方整体拽进 ~2.2cm、外缘撕出台阶——「大腿球包+凹口+腿往中线收拢」根因；渐出 12→7cm 使 δ 在裆−10 即满量收叉口）；**目标 gap 剖面 v3**（控制点 (裆−1, 4.2 叉口水滴形)→(裆−12, 1.2)→(裆−20, 0.9 大腿中段贴合)→(裆−26, 1.8 膝上回松)→(裆−38, 3.5 小腿)，段间 smoothstep 插值——v2 分段目标大腿 2/小腿 3.5 实测主段 1.6~2.0 全程不闭合、二轮 VLM 读作等宽槽 3/10 后加深重设；脚部原生不动〔gap 8~11 是自然外展站姿〕，腿轴带股骨自然内斜 ~2.3°）；同高度共向平移无内部错动 → 水密保持、切片环刚体平移周长不变（围度/地标/裁切面零影响）；vendor 四自检（渐出带下残 gap ≤1.2 / 轴散布 ≤2.5+方向守卫 / **外缘 Lipschitz 裆+1..裆−17 每cm 骤降 ≤0.5〔实测 0.35，旧平移 0.85~1.0〕** / gap 剖面回升 ≤ pre 解剖回升 +0.3〔绝对单调口径与臀褶解剖相悖〕），金标 `test_thigh_gap_closed`（主段 ≤3.2 + 渐出带自裆−8 起单调渐收 + 顶部裆−6 ≥3.0 防过度内收 + 裆−16 ≤2.5）/ `test_outer_silhouette_continuity`（外缘骤降 ≤0.5 + 臀锚 ≥17.8）/ `test_leg_axis_natural` 把守；⑤ **thigh± 为派生径向保形场**（`derived_thigh_targets`，以 payload thigh 站 crotch−3 为中心：逐顶点相对该高度腿环质心的径向增量、**非对称窗——下半半宽 = y_st−膝地标（≈29.5cm，窗在膝站精确归零：对膝围零串扰、大腿根→膝 Δg 单调递减无下段死区）、上半半宽 14cm 经 `_derived_torso_field` 双相在叉上以躯干相延续**（六轮定案：上半 8 时场在裆+1 硬截断而 hips± 平顶恰满幅，两场拼合处 = 陡崖——160/64A「叉上结节+折痕」根因；躯干相 torso_scale 0.28 延续消崖、且弱到不与 hips± 雅可比列共线）、峰值 1.0cm@w=1 ≈ 站点 Δg ~5.2（二轮 0.8→1.0：hips± 平顶窗后臀腿耦合 +9.2/w，thigh± 需更大可达域抵消 hips 权重拖拽）、内侧 ×0.7 衰减 + 内向 gap 预算防线（按各高度实测基间隙自调，叉顶/贴拢带预算→0，满钳不互穿）防病理权重两腿互穿、dy≡0 站高不漂）——替换 MakeHuman 原生 measure-thigh-circ（其 Δgirth 峰值带在 crotch−14 = 它自家 canonical 站，与我们站点错位 11cm：站点灵敏度仅峰值 39%，闭环为凑站点围度把权重推大、中段被撑爆根部反紧——「调大腿围变粗位置太靠下」根因）；vendor 对位自检（峰值带 ∈ 站 ±3.5 + 站点响应 ∈ [3,8] + 腰/膝对角 <0.3 + Δg@crotch−15 ≥2.0 且峰下逐高单调）+ 金标 `test_thigh_derived_band_alignment` 把守；⑥ **knee± 同为派生径向保形场**（`derived_knee_targets`，与 thigh 共享构建器 `_derived_radial_field`：站 = 膝地标、half 15 双侧、amp0 1.25 ≈ 站点 Δg/w ~6.4 对齐原生量级、内侧衰减 0.30 = 间隙预算解析上界 (gap0−floor)/(2·amp0·clamp) = (2.0−0.65)/(2×1.25×2.0)（floor 0.65 = 守卫 0.5 + 采样网格插值边际 0.15，二轮收紧）——w=1 膝内侧间隙 1.40、满钳 w=2 落 ~0.65 ≥ 0.5 守卫防双膝互穿，可达膝围 ~47.2 覆盖典型输入）——整替原生 measure-knee-circ（两类缺陷：canonical 站错位〔峰值 ~45.5 vs 我们膝站 48.06〕；站点窗上缘 y∈[57,66] 是纯内侧 −x 平移剪切边，原生满钳 1.0 时把 ~2cm 膝间隙压到 ~0.01——「双膝贴拢/穿插」根因）；**槽序契约不变**（派生 knee± 追加 targets 末尾，最终槽序仍 [waist±, hips±, thigh±, knee±]，load.ts SLOT_ORDER 零改动）；vendor 对位自检（峰值带 ∈ 站 ±3.5 + 站点响应 ∈ [4,9] + 对 waist/hips/thigh 串扰 <0.3 + w=1 膝内侧间隙 ≥0.5）+ 金标 `test_knee_derived_band_alignment`/`test_knee_gap_floor` 把守；⑦ **hips± 为派生躯干场**（`derived_hips_targets` → `_derived_torso_field`，2026-09-12 二轮整替原生 measure-hips-circ）——原生拉力剖面非单调（y79 0.71→y81 0.66 凹→y83 1.15 跳升 1.7 倍 = 分段构造台阶指纹）叠加基网格裆线平台把臀拉成「平顶+裆上陡崖」（「胯部方形折角」根因）；派生场**平顶窗 core [裆+3, 臀+2] 窗值恒 1**（六轮B 自裆+1 下探罩住转子穹顶与裤+3 以上整段，褶带同进同退）（纯 cos² 峰钉臀站时转子穹顶〔mesh 实际最宽带，裆−2..0〕落窗值 <0.2 处——hips− 只削站带不削穹顶，两者间挖 ~2cm 凹槽 = 一轮 VLM 3/10 根因；平顶使穹顶/站带同进同退，w=1 外缘最大斜率 0.34/cm）+ 下缘 ramp 6 / 上缘 ramp 20（六轮C：上窗 8 在裆+15 归零、waist− 原生下缘够不着 → 裤+8..+18 无人收缩带成 W 形谷@+8/球@+15；拉长上 ramp 平滑交棒 waist−，腰串扰守卫 0.3→0.8——护的是已退役对角三点表，运行时全 4×4 雅可比 0.5cm/w 耦合无害）+ amp0 2.6（站点 Δg/w ~14.3 ∈ [8,20]）+ **proportional 等 % 交付**（六轮B：等径向场对细环交付更狠 %，裆+1..+3 过收 −11.4% vs 需求 −8.2%、下缘戛然 → notch 折痕；改乘 局部环周长/站环周长，站带因子恒 1 响应不变；躯干相下缘再乘 taper 斜坡到 0.55〔平顶满强度送到叉会过收叉带 2~6pp、髋峰相对突出成台阶〕，腿相乘 **`_LEG_RATIO_CAP`=0.12**〔六轮C：等 % 腿相在腿站过收 ~2 倍逼出大 thigh+ 对消、其长下尾把裆−6..−12 马鞍袋带吹胀 +2~4.5%；站点真实需求比（基 99.19/53.46，160 = −2.7%/−11.3% ≈ 0.24、165 ≈ 0）远小于环周长比 ~0.55，截幅后 thigh+ 回落 ~0.26 自持量级、叉带残余 +1.2~1.3% ≈ base 对照水平〕）；**双相轨道**：叉下分腿环（腿相 = 右腿环镜像质心 + 内向 gap 预算 + dx 内向总预算钳）与叉上单环（躯干相 = 全环质心 (0, cz)，中线顶点纯 ±z 镜像天然对称）两相各自独立采样插值、切换带 smoothstep 交叉渐变——单轨插值在叉部 1cm 内 cx 8.3→0.05 跨相内插曾把中线顶点横推 −1.32cm 破镜像、镜像对顶点相向位移 2.59cm/w 互穿；thigh 站串扰 +9.2/w = 臀腿物理重叠（臀底即大腿顶）刻意保留，由运行时差分雅可比联立消解；vendor 守卫 21~24（对位 ±6 平顶宽口径/量级 [8,20]/腰串扰 <0.3/臀腿泄漏 ≤12）+ 25（叉带自裆−4 逐 0.25cm 上扫 w1/w2 内侧 gap ≥0.5）+ 30（fork 带相邻顶点 hips+ 增量按边长归一剪切率 ≤1.5/cm）+ 金标 `test_hips_derived_band_alignment` 把守；⑧ **基网格四解剖 sculpt**（2026-09-12 四项形态报障，全为逐顶点纯位移场、dy≡0、重合顶点位移恒相等 → 水密/拓扑/索引零影响，TS 按名消费零感知）：soften_trochanter 大转子穹顶倒圆（−0.20@裆−1 半宽 4 + 下翼 −0.22@裆−7 半宽 3.5，横向 ramp (|x|−3)/3 只动外侧；一轮 −0.38@裆+3.5 实为在 hips± 挖出的凹槽上加深台阶、二轮移至穹顶轻量化）、sculpt_pelvis_front 耻骨隆突（mons +z 0.9，y 窗 [裆−6,裆+4]、|x| ±3.2、smoothstep(z/2.5) 前向门控防 z=0 平面两侧撕台阶）、sculpt_inguinal_groove 腹股沟斜沟（(7.5,裆+7)→(1.8,裆−1) 径向内拉 0.80 + 法向外弓 0.8·sin(πt) 凸向腹侧，垂直半宽 1.35、两端 15% 弧长渐灭、在 mons 之后施加刻出隆突下边界）、sculpt_leg_muscles 大腿三叶（股四头肌 0.65@0° / 内收肌 0.55@−90° / 腘绳肌 0.45@180°，峰值裆−9、上翼 7/下翼 15）；存活守卫 26~29 量四 sculpt 实测幅度 ∈ 量级带（lobeDg [0.5,3.5] / monsDz [0.4,1.4] / shaveOuter [−0.45,−0.05] / groovDg [0.3,1.1]）防窗错位/锚漂移静默 no-op。
- **运行时链路（`fitting3d/bodymesh/`，渲染与碰撞同源一张真人网格；先验收敛 `priors.ts BODYMESH_PRIOR`，改先验不重打引擎 zip）**：`load.ts`（fetch base.bin 模块级 promise 缓存，异常 console.error + 降级空人台不留白屏；**缓存击穿**：targets.json 先行 `cache:'no-cache'` 拿 `baseSha256` 再以 `?v=<sha8>` 拉 base.bin——vendor 重跑后 URL 变即免陈旧缓存，无指纹旧 meta 回落无参路径）→ `morph.ts`（稀疏叠加 pos = base + Σwᵢ·Δᵢ；地标 = 顶点索引、morph 后重读坐标）→ `align.ts`（**分段线性 y-warp**：sole/ankle→hem/ankle、knee→knee、crotch→crotch、waist→waist 节点精确过点 + 顶上斜率 1.0 外推保 topY 覆盖裤顶 + 逐段斜率卫 0.2~5 + unmapY 逆映射二分；**横向恒等只动 y**。**踝节点路径**（ankle 地标存在时）：踝 → hem−`legBelowHem`(2cm) 精确过点、脚底沿 slope 1.0 保真脚高——真脚（~13cm）整体落在腿场底之下、布料不可达；若按 sole↔hem 地面配对，脚顶被抬进裤口带、脚趾（腿轴前方 ~16cm）按角度 splat 成 θ≈0° 假半径 15+ 污染脚口带碰撞把布推爆。ankle 缺省（合成网格）退化 sole↔hem 恒等）→ `calibrate.ts`（围度闭环：目标 = BodyProfile 四围（thigh 缺省 hip×0.62 @crotch−3），**运行时前向差分实测 4×4 雅可比**——J[j][i]=∂g_j/∂w_i 在当前权重点量测（扰动方向=该站需求方向、**分母须带方向号**——incr/decr 是不同位移场且负方向探针等效改变 −h），原生 waist± 顶点 y 迁移耦合与残留窗重叠一并在内（thigh/knee/hips 派生场均 dy≡0 站高不漂，但 thigh↔hips 臀腿重叠带物理耦合 ~9.2/w 实测在内——差分全量联立求解消解，正因如此 hips± 平顶窗的 thigh 串扰是可解的而非缺陷）；targets.json 预标定三点表是单 target 对角近似测不出耦合，降级为诊断元数据，vendor 金标仍把守对位）；Gauss-Jordan 部分主元联立解**最小权重**（奇异回退对角步进）+ 阻尼 0.8 至 ±1.5%；**分档钳界**：thigh±/knee±/hips± 派生场宽钳 weightClamp ±2.0（cos²/平顶窗场平滑可外推，knee/hips 间隙预算按满钳 2.0 解析设计——hips± 2026-09-12 五轮起派生替换入宽钳档），waist± 原生 target 钳 nativeWeightClamp ±1.0（作者化域 [0,1] 外位移分布未作者化——外推出过膝带 +1.4cm 半径局部鼓包，钳回域内、超域欠量半径当量 ~毫米级视觉不可见；现仅 waist± 留原生）；不收敛 console.warn 残差披露诚实降级。演进见决策日志 §十一）→ `heightfield.ts`（三管 R(y,θ) 表：θ 256 bins × 行距 0.5cm，切片环点 splat + 空 bin 邻域 max 膨胀 + **单 bin 别名坑修复 `fillThetaPits`**〔2026-09-12：叉带截面非星形（8 字环），外缘点因 z 偏移跳相邻 bin 时侧缝向 θ=±90° bin 可能只剩会阴桥点、R ~15 塌 ~3 撕出假台阶拽布料向中线——低于两邻且深 >2cm 的单 bin 局部谷抬到邻 max（与空洞膨胀同向保守），浅谷/宽解剖谷（臀沟 ~20 bin）/空 bin 不动；导出供金标〕；骨盆管裆下钳位延伸 `pelvisBelowCrotch`=3、腿管裆上头带 `legAboveCrotch`=7（单环切片按顶点 x 符号分喂两腿表）、腿管底 `legBottom`=hem−2；数据自检三管顶行非空）→ `build.ts`（**围度标定测点 = mesh 解剖站而非 payload 站线 unmap**〔2026-09-12 口径：y-warp 各段斜率 ≠1（躯干段实测 0.735），unmap 站会漂离解剖站——thigh 钳 `min(unmap, meshCrotch−thighStationDrop)`：引擎 thigh_measure_offset 默认 0 使毗围线与裆线等高、unmap 恰落叉点，该处「腿环」含会阴桥虚高 ~10cm 且切片差 1e-5cm 围度 ±1.4cm 病态敏感，闭环为凑数解出 thigh−≈1.8 把大腿整段挖空（「腿内凹+臀腿交界起伏」主根因）；hips 钉 meshHip 解剖臀峰地标（unmap 实测偏臀峰 +4.7cm，hips± 作用带顶出 vendor 设计域成包络锯齿）；waist/knee 均为对齐节点、偏差 0 无此病〕；Tube 环采样 ringStep=2cm、y 严格降序 + hf 柄闭包；出口 `Mannequin{pelvis, legs[2], topY, bottomY=groundY, sourceMesh}`；站点守卫 yCrotch = min(payload, hip−2)）。
- **适配层契约（seams/pbd/collide 零改动红线）**：`SectionParams` 扩可选 `hf` 柄——`radiusAt` 先查 hf（y 夹端双线性 + θ 环向插值）返回**裸 R**（collisionSkin 由 collideOne 自加），无 hf 回落超椭圆旧路径；`surfaceRadius`（seams 自实现逐管凸支撑 `max(cx·sinθ + R(θ))` 的原点支撑近似）对真人网格角向起伏（腿肚/大腿凸带）留初始残差实测最坏 ~1.5cm——冷启动 preRelaxIters 200 第一帧前推出，终态由集成测试 +skin−0.15 强断言把守。渲染 = `sourceMesh` BufferGeometry 直出（computeVertexNormals），无蒙皮层；BodyProfile 4 键 + 体型 UI + localStorage `ylpattern.body.v1` 契约不变。
- **验收（2026-09-11）**：裸台八图 VLM 逐项通过（std 正/侧/背/斜 + 病理「胖腰粗腿」「腿比盆宽」各正/斜）；着装视与裸台小腿段比例（膝-踝 y-warp 斜率 44/34.562≈1.27 拉伸）复核同日补齐（见决策日志 §十一）。
- **用户两报障修正复验（2026-09-11，数据链 ④⑤）**：std 着装视腿间缝隙 1~2cm 自然；legheavy（大腿围 80>臀围 75）增粗峰值在大腿根部、沿大腿单向收窄；病理着装 5/5；std 着装初审「内缝裂口/裆部破面」两疑点聚焦复查确认为背景色/褶皱阴影误读（与 integration 数字金标一致）。裸台双保证由 Python 金标把守（gap ≤3.2 / 峰值带 ∈ 站 ±3.5）。
- **腿部扭曲三轮修复复验（2026-09-11，数据链 ⑤⑥ + calibrate 钳位分层）**：三档 VLM——est（典型 68.5/91/54/43）5/5 正常；legheavy/std 的「髋侧球包 / 腿向中线收拢」读数经 node 数字裁决为布料伪影或误报（三带围度局部峰全 0.00、左右镜像 <0.1cm、大腿内侧间隙较基底最多收 0.22cm；成衣围与人体围的富余/不足属版型松量非人台缺陷）。裸台保证由 Python 金标把守。史实与 harness 坑（analyze_image 缓存碰撞、冷启动时序）见决策日志 §十一。
- **腿部形状四轮修复复验（2026-09-12，运行时测点钉解剖站 + fillThetaPits）**：用户报「腿内凹 + 臀腿交界起伏」（腰 66/臀 90/腿 54/膝 35）——node 数字诊断坐实双根因（见运行时链路两处 2026-09-12 注：thigh 测点落裆叉致标定解出 thigh−=1.83 挖空大腿、高度场 θ-bin 别名坑 8.9cm/0.5cm 假台阶，另 hips 站漂移 +4.7cm 为锯齿贡献项）；修复后 thigh+=0.72 正常收敛（残差 0.00%）、渲染外缘超 0.5cm/cm 台阶 0 步、叉带包络连续。金标 bodymesh fillThetaPits 单测/thigh 钳站测 + integration 包络坑守卫把守。教训：集成测试只断言拓扑/有限性不钉构型——**标定垃圾能全绿穿过**，形状类报障必须先 node 数字复算（离线诊断脚本量出 thigh−=1.83 一锤定音）。
- **六轮根治复验（2026-09-12，「160/64A 叉上结节+折痕」根治：vendor 派生场重排，用户拍板「根治」方向）**：三轮 A/B 定案三根因三层修——**A 叉上陡崖**：thigh± 上半窗 8 在裆+1 硬截断 vs hips± 平顶恰满幅，拼合 = 陡崖（−2.1cm/2cm）→ thigh± 改 `_derived_torso_field` 双相（叉上躯干相 0.28 延续）；**B 等 % 化**：等径向场对细环交付更狠 % 挖 notch → hips± proportional（环周长比因子 + 腿相独立轨道）；**C W 形/马鞍袋**：hips± 上窗 8 死于裆+15、waist− 够不着 → 裤+8..+18 无人收缩带（W 谷@+8/球@+15）→ 上 ramp 20 + 腰串扰守卫 0.8；等 % 腿相过收 ~2 倍逼出 thigh+ 0.89 对消、其长下尾把裆−6..−12 吹胀 +2~4.6%（165 正面「马鞍袋」放大主凶）→ `_LEG_RATIO_CAP` 0.12（0.40 实测仍 +3.5%；按预标定表建模 thigh+(c)=(w_hips·c·8.4+demand)/9.8 反解带内 ≤1.5%）。**关键校准**：vendor lm.hip 在裆+5.5（非臆断 +20）——叉→站 span 仅 8cm，hips± 上窗 8 恰好死在裆+15 是 W 形直接证据；calibrateWeights 在 mesh 原生高度测（无 applyVertical），诊断打印混两系 = 假警报（_diag_iter 复刻求解器证明 2-3 轮收敛、it2 = 生产权重精确复现）。**终态（sha 4a9e3e34）**：165 解 {waist−0.32, hips−0.70, thigh+0.26, knee+0.11}、160 {waist−0.51, hips−0.81, thigh−0.08, knee−0.09}（对消结构消失：160 腿相翻负 = 真实需求方向）；叉带 165 +1.2~1.3% / 160 −0.4~−1.2%（≈ base 直显对照）、中带 +2..+18 平滑单谷无 W；金标 pytest 28 + vitest 66 全绿零同步。**VLM 终审（工业人台措辞 + 像素网格同仪器）**：165/160 斜 45° 自然~轻度（1-3/10）、160 裆轻度 2-3 ≈ base、165 裆 5-6 vs **base 直显对照 4**（MakeHuman 转子穹顶原生台阶）——残余 = base 解剖 + 预设体型真实差（hip 90/thigh 54 梨形比 0.60 vs MakeHuman 0.55），最终剪影由 girth 目标决定、任何场形/基网格改动不改变总轮廓（求解器收敛同目标），平顶 core A/B 与 cos² VLM 判不出（±2 分仪器噪声主导）按数字退回 cos²。方法论：VLM 报缺陷先 base 直显对照定基线（区分 morph 层 vs 数据层）；读数差 <仪器噪声（同图重读 ±1.5~2 分）不迭代；验收扫描基准系必须与渲染系一致（native vs payload 高度混测 = 假回归/假达标双向坑）。
- **四项解剖形态修正复验（2026-09-12，两轮；①大转子软化/②裆部体积/③腹股沟结构线/④腿部肉感）**：一轮实施（四 sculpt + hips± 派生纯 cos² 峰 + adduct v2）VLM 复验 3/3/5/5 未过 → 数字复算定根因（hips± 峰钉臀站、mesh 最宽带在转子穹顶裆−2..0 落窗值 <0.2 处——hips− 0.53 只削站带 −1.38 不削穹顶 −0.18，两者间挖 ~2cm 凹槽；gap 主段 1.6~2.0 全程不闭合读作等宽槽）→ 二轮 hips± 平顶窗双相轨道重写 + adduct v3 + groove 加深外弓 + lobes 0.65/0.55/0.45 + thigh± amp 1.0。三视图 VLM 复验过：正面 6/5/7/7、3/4 侧 8.5/8/6.5/8、裆特写 7/8/6/6.5——①过（3/4 侧「平滑连续圆弧未见折角/平台」8.5，正面残余仅髂嵴交界轻度折角 6）、②过（mons 7/下腹过渡 8「饱满自然无扁平塌陷」）、③过（特写 8「清晰斜向、走向正确」——正面远景 5 报「走向水平平直」与构造矛盾：沟轴 (7.5,裆+7)→(1.8,裆−1) 与水平夹角 ~55°、深 0.80cm 在全身截图 ≈2px，判远景分辨率误读非几何缺陷，数字复算裁决不改码）、④过（正面 7/3/4 侧 8「锥度比例合理、膨隆柔和」）。残留判可接受：叉顶「双坑」= 两条独立沟内侧端 + mons 隆突的真实几何（两沟中心相距 3.6cm 不重叠，与真实解剖一致）；中段肌肉分群细节略欠 = 静态网格无皮肤软组织渲染的固有限度。裸台保证由 Python 金标 15 例把守（hips 对位/叉带 sweep/knee 间隙 floor/外缘连续/sculpt 幅度带）。
