...
3D 试穿 fitting 出口已程序化（`exporters/fitting.py` 的 `build_fitting_payload(ctx, m)`，纯 stdlib：整版全局系 cm Y-up 序列化前后片净边链/边名角色表/站点围度/腰头标量；`PatternPiece` 尾部新增 `origin/frame` 默认字段随片携带，pieces.py×2 + cutter.py 三处重建点透传，金标钉死）。双通道 `POST /api/draft/fitting` 与 `engine_glue._fitting` 同步（tests/test_engine_glue.py 全等）。金标：tests/test_fitting_payload.py。前端 2026-09-12 裁撤试穿只留人台显示（body 站点作人台纵向对齐锚、pieces 闲置，出口/端点/双通道不动）。**坐标系/人台/体型分离口径权威 = §10.11**，演进见决策日志 §十一。
...
### 10.11 3D 人台显示（fitting 出口 + 真人网格人台；试穿链 2026-09-12 退役）

**定位**：右栏 3D 主视图常驻显示真人网格人台（进系统即有），按体型围度 + 尺寸单站点 morph 塑形——当前聚焦「人台自然 + 自由调节尺寸不变形」。试穿（前后片当布料缝到人台 + PBD 解算 + 碰撞）2026-09-12 前端整体退役（用户裁决先解决人台问题；退役清单与演进史见决策日志 §十一，git 历史可复）；引擎 JSON 出口照旧，恢复试穿 = 前端 revert、引擎零改动。
...
- **边角色表**（缝合配对键，随试穿退役仅供恢复参考）：front `waist:top_chain, hem:hem, mouth/fly_*:free, rise/inseam/side:seam`；back `waist:top_chain, hem:hem, cb/inseam/side:seam`。`girth_finished` 均为成衣量（waist/hip 整圈、thigh/knee/hem 单腿；松量读数控件已随试穿退役，字段保留），**人台围度不来自 payload**。
...
**前端（webapp/frontend/src/fitting3d/，依赖仅 three）**：
...
- **模块图**：`priors.ts`（唯一先验收敛点：松量/体型/网格人台先验，全为非引擎数据）→ `bodyProfile(t)` → `bodymesh/`（真人网格人台唯一路径：load/morph/align/calibrate/build，见下文「真人网格人台」节）→ `useMannequin.ts`（构建生命周期 hook：payload body 站点 + 围度 → 人台，静态无 rAF；构建失败 console.error + 回 idle 不炸 React 树）→ `Fitting3DView.tsx`/`BodyProfileDrawer.tsx`（人台渲染 + 体型抽屉 + 视角预设/截图/显示开关；`?bodymesh=raw|base` 网格直显诊断——分层排查失真引入层：源数据/vendor 脚本/运行时，零处理不走构建链，也不触发 payload 链）。**试穿链 2026-09-12 整体退役**：mesh/seams/pbd/structureLines/heatmap/mannequin 适配层/bodymesh/heightfield/useFittingSolver、RENDER_GARMENT 与 ?garment 开关、松量读数/热力图/透明度控件、delaunator 依赖；缝合配对/摆位/PBD/碰撞细节不再维护于本档（退役清单见决策日志 §十一）。「生成」仍取 payload——body 站点是人台纵向对齐锚，pieces 字段闲置。
- **测试**：`fitting3d.test.ts`（estimateBody 金标/体型库）+ `bodymesh.test.ts`（合成网格金标：slice 切片/morph 稀疏叠加/align 分段 y-warp（含踝节点路径）/calibrate 差分雅可比反演 + 耦合联立 + 分档钳界/buildMeshMannequin 出口契约 + thigh 钳站测（BODY 带 thigh y=crotch 复刻引擎 thigh_measure_offset=0 口径，站钳回 crotch−3 后闭环收敛、站点围度=目标）——node 无 fetch/public，**真数据金标在 Python 侧** test_vendor_bodymesh.py）+ `fitting3d.integration.test.ts`（真资产人台端到端：node:fs 读 public/bodymesh 走与浏览器同一 parseBin→buildMeshMannequin 链，断言顶底序/位置有限/站点围度达标（waist/knee ±2%——thigh 标定测点钳在 mesh 坐标裆叉下，payload 侧重建需复刻分段对齐、叉环处切片含裆桥虚高〔实测 57.8〕正是钳位理由，钳位行为由合成金标把守））。fixture 重生成命令见 integration 文件头；fixture 用静态 `import fx from './_fixture_fitting.json'`（tsconfig 开 `resolveJsonModule`）——**产品代码勿用 `node:fs`**：tsc -b 浏览器 lib 无其类型，`npm run build` 的 tsc 阶段报 TS2307（测试文件内可以 `// @ts-expect-error` 紧贴 import 行压制）。
...
- **运行时链路（`fitting3d/bodymesh/`，渲染直出一张真人网格（碰撞链 2026-09-12 退役）；先验收敛 `priors.ts BODYMESH_PRIOR`，改先验不重打引擎 zip）**：`load.ts`（fetch base.bin 模块级 promise 缓存，异常 console.error + 降级空人台不留白屏；**缓存击穿**：targets.json 先行 `cache:'no-cache'` 拿 `baseSha256` 再以 `?v=<sha8>` 拉 base.bin——vendor 重跑后 URL 变即免陈旧缓存，无指纹旧 meta 回落无参路径）→ `morph.ts`（稀疏叠加 pos = base + Σwᵢ·Δᵢ；地标 = 顶点索引、morph 后重读坐标）→ `align.ts`（**分段线性 y-warp**：sole/ankle→hem/ankle、knee→knee、crotch→crotch、waist→waist 节点精确过点 + 顶上斜率 1.0 外推保 topY 覆盖裤顶 + 逐段斜率卫 0.2~5 + unmapY 逆映射二分；**横向恒等只动 y**。**踝节点路径**（ankle 地标存在时）：踝 → hem−`legBelowHem`(2cm) 精确过点、脚底沿 slope 1.0 保真脚高——真脚（~13cm）整体落在踝节点之下（人台按穿裤形态对齐；历史碰撞口径见决策日志，链退役后对齐语义保留）。ankle 缺省（合成网格）退化 sole↔hem 恒等）→ `calibrate.ts`（围度闭环：目标 = BodyProfile 四围（thigh 缺省 hip×0.62 @crotch−3），**运行时前向差分实测 4×4 雅可比**——J[j][i]=∂g_j/∂w_i 在当前权重点量测（扰动方向=该站需求方向、**分母须带方向号**——incr/decr 是不同位移场且负方向探针等效改变 −h），原生 waist± 顶点 y 迁移耦合与残留窗重叠一并在内（thigh/knee/hips 派生场均 dy≡0 站高不漂，但 thigh↔hips 臀腿重叠带物理耦合 ~9.2/w 实测在内——差分全量联立求解消解，正因如此 hips± 平顶窗的 thigh 串扰是可解的而非缺陷）；targets.json 预标定三点表是单 target 对角近似测不出耦合，降级为诊断元数据，vendor 金标仍把守对位）；Gauss-Jordan 部分主元联立解**最小权重**（奇异回退对角步进）+ 阻尼 0.8 至 ±1.5%；**分档钳界**：thigh±/knee±/hips± 派生场宽钳 weightClamp ±2.0（cos²/平顶窗场平滑可外推，knee/hips 间隙预算按满钳 2.0 解析设计——hips± 2026-09-12 五轮起派生替换入宽钳档），waist± 原生 target 钳 nativeWeightClamp ±1.0（作者化域 [0,1] 外位移分布未作者化——外推出过膝带 +1.4cm 半径局部鼓包，钳回域内、超域欠量半径当量 ~毫米级视觉不可见；现仅 waist± 留原生）；不收敛 console.warn 残差披露诚实降级。演进见决策日志 §十一）（三管高度场 heightfield.ts/fillThetaPits 随碰撞链退役 2026-09-12，史实见决策日志）→ `build.ts`（**围度标定测点 = mesh 解剖站而非 payload 站线 unmap**〔2026-09-12 口径：y-warp 各段斜率 ≠1（躯干段实测 0.735），unmap 站会漂离解剖站——thigh 钳 `min(unmap, meshCrotch−thighStationDrop)`：引擎 thigh_measure_offset 默认 0 使毗围线与裆线等高、unmap 恰落叉点，该处「腿环」含会阴桥虚高 ~10cm 且切片差 1e-5cm 围度 ±1.4cm 病态敏感，闭环为凑数解出 thigh−≈1.8 把大腿整段挖空（「腿内凹+臀腿交界起伏」主根因）；hips 钉 meshHip 解剖臀峰地标（unmap 实测偏臀峰 +4.7cm，hips± 作用带顶出 vendor 设计域成包络锯齿）；waist/knee 均为对齐节点、偏差 0 无此病〕；出口 `MeshMannequin{topY, bottomY=groundY, sourceMesh}`（Tube/hf 柄随碰撞链退役）；站点守卫 yCrotch = min(payload, hip−2)）。
- **（史实存档）适配层契约已随试穿链退役（2026-09-12）**：SectionParams.hf/radiusAt/sectionAt/surfaceRadius 与「seams/pbd/collide 零改动」红线消费方全灭，mannequin.ts/heightfield.ts 删除；渲染 sourceMesh BufferGeometry 直出口径不变；BodyProfile 4 键 + 体型 UI + localStorage `ylpattern.body.v1` 契约不变。继承口径见退役前 git 历史与决策日志 §十一。
...
- **腿部形状四轮修复复验（2026-09-12，运行时测点钉解剖站；fillThetaPits 已随碰撞链退役）**：用户报「腿内凹 + 臀腿交界起伏」（腰 66/臀 90/腿 54/膝 35）——node 数字诊断坐实双根因（见运行时链路两处 2026-09-12 注：thigh 测点落裆叉致标定解出 thigh−=1.83 挖空大腿、高度场 θ-bin 别名坑 8.9cm/0.5cm 假台阶，另 hips 站漂移 +4.7cm 为锯齿贡献项）；修复后 thigh+=0.72 正常收敛（残差 0.00%）、渲染外缘超 0.5cm/cm 台阶 0 步、叉带包络连续。金标把守现状（2026-09-12 裁撤试穿后）：bodymesh thigh 钳站测在役；fillThetaPits 单测与 integration 包络坑守卫随碰撞链退役（合成金标覆盖钳位、真实包络回归改由 ?bodymesh=raw|base 三点对比诊断兜底）。教训：集成测试只断言拓扑/有限性不钉构型——**标定垃圾能全绿穿过**，形状类报障必须先 node 数字复算（离线诊断脚本量出 thigh−=1.83 一锤定音）。
