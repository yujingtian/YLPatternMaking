...
- **测试**：`bodymesh.test.ts`（parseBaseBin 布局契约含尾部残留/索引越界防爆 + morph w=0 逐位恒等/稀疏叠加/源数组不被改写 + **height.ts 纯函数**：heightCm 正负两支独立/weightFor 互逆 + 出域钳制/stationFactor 恒等与预设因子 + **真产物冒烟**：盘上读现货 base.bin+targets.json，计数对账/六站齐全/14 场名/heightInfo 实测契约（基高域、ΔH 双向、身高场覆盖全网格）/身高场方向与脚底锚地/官方 waist+ 与 calf+ 自家站方向响应；devDep @types/node，`import.meta.dirname` 定位）；**穿台金标三件（2026-09-18 十期）**：`dressfield.test.ts`（placement 穿台三原语合成场单元：shiftPositionsY 只动 Y/不改写原数组；nearestRingBoundary 环内点法线朝外〔±0.05 跨界验证〕/环外近壳精确距〔16 边形顶点恰在 +x 向，(6,0) 最近边界 = 顶点 d=1.0 精确〕/双环腿间点归最近环；buildLegAxisFromRings fork 搜索/线性插值/域外钳端/无分离行与不足 2 行两 throw + yMin 场负 y 域有环/行号与 forkY 按 yMin 换算 + ankleY 止踝〔脚环 r+|cz| 放大口径不进 rProf〕两用例）、`settle.test.ts`（状态机全迹线——字面量 stub sim + opts.probe 注入：hold 不动钉/frontness lerp 一次下放三钉 y 精确/持续穿透钳 maxDrop→fault→confirm→settle→sim.settled→done→report 全字段/report 接触三态与 −∞ 钳 probeMargin/wake 清假 settled 续预算/maxTotalFrames 硬顶/frozen 直出；probeCrotch 合成单环场壳口径 + yLift 行选择；坑：dropB = −hB 得 −0，toBe(0) 严格比较失败须 toBeCloseTo）、`dress.test.ts`（真 base.bin + fixture 整裤全管线集成 = Fitting3DView dress 分支的无渲染镜像：anchorLift canary ∈(7.0, 8.3) + 人台腰地标 vendor 契约/θ 对齐 canary〔front L rise 末顶点 z>0、back L cb 末顶点 z<0〕/初摆位 pointInRings 计数 0/落位 done 无 NaN 裆四尖两两 <1.0/dropF ≥2.0 且 ∈[0, maxDrop+ε]/minY ≥ −1e-6〔掉裆后裤脚及地判读〕/**同输入双跑 DressReport 逐字段相等 + pos 数组相等**〔确定性红线——单跑 floor 比较无意义〕/hips+ 负松量探索例宽松断言〔偏小是读数不是错误〕；超时第三参——vitest 无配置文件默认 5s，长集成单跑 ~230s、**全量并行负载下 286s**，双跑用例给 480_000/探索例 300_000）、`heatmap.test.ts`（2026-09-18 热力图：合成 sim+环场——gap 环心 5·cos(π/16)−CORE_SKIN 精确/超 margin 与空环行钳 gapMax 松端/yLift 行换算逐位同环心/field null 全 0；strain 逐顶点参与约束 signed max、rest=0 守卫、无约束顶点 NaN→0、dist 局部号+offset〔与 drape 同口径——首版用例误传全局号即炸，口径用例钉死〕；heatColor 两通道端点/中点/越界钳，strain 方向钉死 +max 拉伸=红〔首版 (s+max)/2max 把拉伸映到蓝端是反向 bug，已修〕）。引擎金标 `python -m pytest tests/test_fitting_payload.py tests/test_vendor_bodymesh.py -q` 照旧。
