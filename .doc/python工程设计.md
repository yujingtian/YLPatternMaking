...
- **数据链（git 内自足）**：`vendor/makehuman/`（base.obj + macro 2 + measure ±8 targets + CC0 全文 + PROVENANCE.md 记 repo/tag/commit/sha256；`.gitattributes -text` 禁 EOL 转换保哈希可校验）→ `scripts/vendor_makehuman.py` → `webapp/frontend/public/bodymesh/{base.bin, targets.json}`；Python 金标 `tests/test_vendor_bodymesh.py` 11 例（数据缺席整文件 skip）。vendor 数据级坑（先探针 dump 定案再动手）：① base.obj 为 **A-pose 绑定姿势**——`desplay_legs` 每顶点 XY 平面旋转（pivot=中轴 (0, 裆+6)，权重 = y 向 smoothstep（裆±8 blend 带）× |x| 向（≤2 会阴中带刚性→≥7 全量，防裆顶点被左右反向旋转撕裂）），**targets 增量随各自顶点同旋转**（morph 语义 = 旋转后场上叠加），自检腿轴竖直残差 <1.2cm；② 旋转使腿微缩脚下探，re-zero 抬升 +3.1cm——height/粗裁/精裁自检基准随之重算；③ 臀地标检测带上移 [裆+6, +14] 避开裆叉喇叭带（双腿合并叉环伪围度 ~115 > 真臀 ~98）；④ **腿内收 `adduct_thighs`**（desplay 只对直腿轴、pivot 在裆上，裆下腿轴间距 ~19cm 原样保留：膝上 gap 5.3~6.5、膝下 6.2~7.7）——逐 1cm 量右腿环内侧 gap，单侧共向平移 δ=max(0, gap−target)/2，分段目标大腿 2 / 小腿 3.5（±2cm 线性过渡 + 踝上 6cm 渐入，脚部原生不动〔gap 8~11 是自然外展站姿〕），δ 全程 1.6~2.2 平稳带保腿轴竖直；同高度共向平移无内部错动 → 水密保持、切片环刚体平移周长不变（围度/地标/裁切面零影响），金标 `test_thigh_gap_closed` 把守；⑤ **thigh± 为派生径向保形场**（`derived_thigh_targets`，以 payload thigh 站 crotch−3 为中心：逐顶点相对该高度腿环质心的径向增量、cos² 窗半宽 18cm、峰值 0.8cm@w=1 ≈ 站点 Δg 4cm 与原生同量级、内侧 ×0.7 衰减防病理权重两腿互穿、dy≡0 站高不漂）——替换 MakeHuman 原生 measure-thigh-circ（其 Δgirth 峰值带在 crotch−14 = 它自家 canonical 站，与我们站点错位 11cm：站点灵敏度仅峰值 39%，闭环为凑站点围度把权重推大、中段被撑爆根部反紧——「调大腿围变粗位置太靠下」根因）；vendor 对位自检（峰值带 ∈ 站 ±3.5 + 站点响应 ∈ [3,8] + 腰/膝对角 <0.3）+ 金标 `test_thigh_derived_band_alignment` 双把守。
- **运行时链路（`fitting3d/bodymesh/`，渲染与碰撞同源一张真人网格；先验收敛 `priors.ts BODYMESH_PRIOR`，改先验不重打引擎 zip）**：`load.ts`（fetch base.bin 模块级 promise 缓存，异常 console.error + 降级空人台不留白屏）→ `morph.ts`（稀疏叠加 pos = base + Σwᵢ·Δᵢ；地标 = 顶点索引、morph 后重读坐标）→ `align.ts`（**分段线性 y-warp**：sole/ankle→hem/ankle、knee→knee、crotch→crotch、waist→waist 节点精确过点 + 顶上斜率 1.0 外推保 topY 覆盖裤顶 + 逐段斜率卫 0.2~5 + unmapY 逆映射二分；**横向恒等只动 y**。**踝节点路径**（ankle 地标存在时）：踝 → hem−`legBelowHem`(2cm) 精确过点、脚底沿 slope 1.0 保真脚高——真脚（~13cm）整体落在腿场底之下、布料不可达；若按 sole↔hem 地面配对，脚顶被抬进裤口带、脚趾（腿轴前方 ~16cm）按角度 splat 成 θ≈0° 假半径 15+ 污染脚口带碰撞把布推爆。ankle 缺省（合成网格）退化 sole↔hem 恒等）→ `calibrate.ts`（围度闭环：目标 = BodyProfile 四围（thigh 缺省 hip×0.62 @crotch−3），**运行时前向差分实测 4×4 雅可比**——J[j][i]=∂g_j/∂w_i 在当前权重点量测（扰动方向=该站需求方向、**分母须带方向号**——incr/decr 是不同位移场且负方向探针等效改变 −h），cos² 窗重叠耦合（knee+ 向 thigh 站、thigh 场向 hips 站溢出）与原生 target 顶点 y 迁移效应一并在内；targets.json 预标定三点表是单 target 对角近似测不出耦合，降级为诊断元数据，vendor 金标仍把守对位）；Gauss-Jordan 部分主元联立解**最小权重**（奇异回退对角步进）+ 阻尼 0.8 至 ±1.5%；**分档钳界**：thigh± 派生径向场宽钳 weightClamp ±2.0（cos² 场平滑可外推），waist/hips/knee± 原生 target 钳 nativeWeightClamp ±1.0（作者化域 [0,1] 外位移分布未作者化——外推出过膝带 +1.4cm 半径局部鼓包，钳回域内、超域欠量半径当量 ~毫米级视觉不可见）；不收敛 console.warn 残差披露诚实降级。演进见决策日志 §十一）→ `heightfield.ts`（三管 R(y,θ) 表：θ 256 bins × 行距 0.5cm，切片环顶点 splat + 邻域插值；骨盆管裆下钳位延伸 `pelvisBelowCrotch`=3、腿管裆上头带 `legAboveCrotch`=7（单环切片按顶点 x 符号分喂两腿表）、腿管底 `legBottom`=hem−2；数据自检三管顶行非空）→ `build.ts`（Tube 环采样 ringStep=2cm、y 严格降序 + hf 柄闭包；出口 `Mannequin{pelvis, legs[2], topY, bottomY=groundY, sourceMesh}`；站点守卫 yCrotch = min(payload, hip−2)）。
