...
**前端（webapp/frontend/src/fitting3d/，Fitting3DView + bodymesh/{bin,morph,types,slice}，依赖仅 three）**：
- **现行链路**：`Fitting3DView`（three 惰性分包；场景/格网/OrbitControls/视角预设/截图/显示开关；无 props、零 payload）→ `loadBodyMesh()`（bin.ts，模块级 promise 缓存）：fetch `targets.json`（**`cache:'no-cache'` 带 ETag 再验证——它是击穿链的链头，裸 fetch 会被浏览器启发式缓存钉死旧指纹、指向旧 base.bin，vendor 重跑后整链失效**）→ 以其 `baseSha256` 指纹拉 `base.bin?v=<sha8>`（vendor 重跑后指纹必变即击穿浏览器缓存）→ `parseBaseBin` 二进制解析（布局头注 + 尾部残留/索引越界防爆）+ 与 targets.json 计数对账 → BufferGeometry 直显 → 滑杆 onChange：双极值映射场权重（正推 `site+`、负推 `site−`，两场独立作者化非反对称）→ `morphPositions`（稀疏叠加 pos = base + Σwᵢ·Δᵢ，签名只吃 `{positions, targets}` 结构）in-place 写 position attr + computeVertexNormals → `readGirth` 四站围度读数。初始机位按裁切面高（腰+15）定标；加载失败 Empty 提示。**对齐/围度闭环/钳位不复位**：滑杆是裸权重，不经过 retired 链任何守卫。
...
- **资产与许可**：数据源 `vendor/makehuman/`（base.obj + rigs/{default.mhskel 骨架, default_weights.mhw 蒙皮权重} + targets + CC0 全文 + PROVENANCE.md 记 repo/tag/commit/sha256；`.gitattributes -text` 禁 EOL 转换保哈希可校验）；运行时消费 `public/bodymesh/{base.bin,targets.json}`（LICENSE.ASSETS.md 同目录）。许可红线（用户拍板）：仅商用安全许可（CC0/MIT/Apache）、MakeHuman 只借 CC0 数据绝不移植 AGPL 代码、SMPL 出局。`scripts/vendor_makehuman.py`（2026-09-13 定型为**切割+站直姿势链**：站直姿势 = 官方 rigs 数据 clean-room LBS〔大腿绕髋 θ_t、小腿绕移动后膝补 θ_k、脚绕移动后踝反补 −θ_c，臀带由 upperleg01 权重自然携带〕+ 下半身切割 + 官方 12 场重映射与增量随帧旋转，`--pose apose` 可退回 A-pose 复现口径；腿去外张/内收/四解剖雕塑/派生场/预标定矩阵全部退役——自研姿势手术撕裂臀线教训，「零姿势修改」口径同日退役；Python 金标 `tests/test_vendor_bodymesh.py` 把守——含水密/地标带/官方场方向响应/站直守卫〔腿轴竖直/膝铅垂/脚平底/左右镜像〕与官方场增量镜像对称〔防随帧旋转线性角未归一的单侧翻转——atan2+π 原值对位置是整周等价、乘部分权重后即翻转，vendor exit 13 域守卫同防〕）。
- **测试**：`bodymesh.test.ts`（parseBaseBin 布局契约含尾部残留/索引越界防爆 + morph w=0 逐位恒等/稀疏叠加/源数组不被改写 + **真产物冒烟**：盘上读现货 base.bin+targets.json，计数对账/六站齐全/官方 waist+ 与 calf+ 自家站方向响应；devDep @types/node，`import.meta.dirname` 定位）。引擎金标 `python -m pytest tests/test_fitting_payload.py tests/test_vendor_bodymesh.py -q` 照旧。
